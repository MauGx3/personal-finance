...
class DjangoFeatureRegistry:
    ...
    def register_viewsets(
        ...
        try:
            ...
            self._logger.debug(f"Django feature '{feature_name}' registered successfully")
            
        except (ImportError, AttributeError, Exception) as e:
            feature_info.import_error = str(e)
            self._logger.debug(f"Django feature '{feature_name}' unavailable: {e}")
            ...
            if required:
                self._logger.error(f"Required Django feature '{feature_name}' is unavailable: {e}")
                raise ImportError(f"Required Django feature '{feature_name}' cannot be imported: {e}")
    ...
    def is_available(self, feature_name: str) -> bool:
        ...
        return self._features.get(feature_name, DjangoFeatureInfo("", "", [], False)).is_available
    
    def get_viewset(self, feature_name: str, viewset_name: str, default: Any = None) -> Any:
...
def register_optional_viewsets(
    ...
    """
    return django_registry.register_viewsets(feature_name, module_path, viewset_names, required=False)
...
def register_required_viewsets(
    ...
    """
    return django_registry.register_viewsets(feature_name, module_path, viewset_names, required=True)


def get_viewset(feature_name: str, viewset_name: str, default: Any = None) -> Any:
