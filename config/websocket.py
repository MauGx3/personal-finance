...
class WebSocketHandler:
    ...
    async def handle_subscribe_asset(self, payload: Dict[str, Any]):
        ...
        response = encode_message(
            "subscription_confirmed", {"type": "asset", "symbol": symbol}
        )
    ...
    async def handle_unsubscribe_asset(self, payload: Dict[str, Any]):
        ...
        if not symbol:
            await self.send_error(
                "Symbol is required for asset unsubscription"
            )
            return

        await connection_manager.unsubscribe_from_asset(
            self.connection_id, symbol
        )

        response = encode_message(
            "unsubscription_confirmed", {"type": "asset", "symbol": symbol}
        )
        await self.send({"type": "websocket.send", "text": response})
    ...
    async def handle_subscribe_portfolio(self, payload: Dict[str, Any]):
        ...
        if not self.is_authenticated:
            await self.send_error(
                "Authentication required for portfolio subscription"
            )
            return
        ...
        if not portfolio_id:
            await self.send_error(
                "Portfolio ID is required for portfolio subscription"
            )
            return
        ...
        await price_feed_service.subscribe_to_portfolio(
            self.connection_id, portfolio_id
        )

        response = encode_message(
            "subscription_confirmed",
            {"type": "portfolio", "portfolio_id": portfolio_id},
        )
        await self.send({"type": "websocket.send", "text": response})
    ...
    async def handle_unsubscribe_portfolio(self, payload: Dict[str, Any]):
        ...
        if not portfolio_id:
            await self.send_error(
                "Portfolio ID is required for portfolio unsubscription"
            )
            return
    ...
    async def send_error(self, message: str):
        ...
        await self.send({"type": "websocket.send", "text": error_msg})
