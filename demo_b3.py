...
Emolumentos                                                8,00
    ...
    print(f"   💸 Total Buy Value: R$ {result.summary['total_buy_value']:,.2f}")
    print(f"   💰 Total Sell Value: R$ {result.summary['total_sell_value']:,.2f}")
...
31/03/2024 Saldo final                                                  118.010,25
    ...
    if result.extract_entries:
        ...
        for i, entry in enumerate(result.extract_entries[:5], 1):
            ...
            print(f"      Credit: {credit_str:<12} Debit: {debit_str:<12} Balance: {balance_str}")
            print()
...
def main():
    ...
    try:
        ...
        print("      python import_b3.py --file your_document.txt --auto-detect")
        print("   2. Export results to JSON for further processing:")
        print("      python import_b3.py --file document.txt --auto-detect --output results.json")
