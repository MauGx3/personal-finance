...
def demo_stock_analysis():
    ...
    try:
        ...
        print(f"  P/E Ratio: {ratios.pe_ratio:.1f}" if ratios.pe_ratio else "  P/E Ratio: N/A")
        print()
        ...
        try:
            ...
            sample_prices = pd.Series(np.random.uniform(95, 105, 50), index=dates)
            sample_high = sample_prices + np.random.uniform(1, 3, 50)
            sample_low = sample_prices - np.random.uniform(1, 3, 50)
            sample_volume = pd.Series(np.random.randint(1000000, 5000000, 50), index=dates)
            ...
            print(f"  Bollinger Upper (last): ${bb_data['upper'].iloc[-1]:.2f}")
            print(f"  Bollinger Lower (last): ${bb_data['lower'].iloc[-1]:.2f}")
            
            # Stochastic Oscillator
            stoch_data = TechnicalIndicators.stochastic_oscillator(sample_high, sample_low, sample_prices)
            print(f"  Stochastic %K (last): {stoch_data['%K'].iloc[-1]:.1f}")
            
            # Williams %R
            williams_r = TechnicalIndicators.williams_r(sample_high, sample_low, sample_prices)
            print(f"  Williams %R (last): {williams_r.iloc[-1]:.1f}")
            
            print()
            
        except ImportError:
            print("pandas/numpy not available - skipping technical indicators demo\n")
        ...
        print("Some dependencies are not available, but the basic structure is implemented.")
