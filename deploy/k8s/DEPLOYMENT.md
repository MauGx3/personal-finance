...
kubectl wait --for=condition=Ready pod -l app=personal-finance --timeout=300s
```
...
kubectl port-forward service/personal-finance-web 8000:80
...
kubectl exec -it deployment/personal-finance-web -- \
...
```bash
# View all resources
kubectl get all -l app=personal-finance

# Check pod status
kubectl get pods

# View logs from web pods
kubectl logs -f deployment/personal-finance-web

# Check celery workers
kubectl logs -f deployment/personal-finance-celery-worker

# Check celery beat scheduler
kubectl logs -f deployment/personal-finance-celery-beat

# View PostgreSQL logs
kubectl logs personal-finance-pg-0

# View Redis logs
kubectl logs deployment/personal-finance-redis
```
...
minikube service personal-finance-web --url
...
```bash
# Build and tag image
docker build -t personal-finance:v1.0.0 .

# Tag for your registry
docker tag personal-finance:v1.0.0 \
  <your-registry>/personal-finance:v1.0.0

# Push to registry
docker push <your-registry>/personal-finance:v1.0.0
```
...
kind: Secret
metadata:
  name: personal-finance-pg-credentials
stringData:
  username: personal_finance_user
...
kind: ConfigMap
metadata:
  name: personal-finance-config
data:
  DEBUG: "False"
  DJANGO_SETTINGS_MODULE: "config.settings.prod"
  DJANGO_ALLOWED_HOSTS: "your-domain.com,personal-finance-web"
...
images:
  - name: personal-finance
    newName: <your-registry>/personal-finance
    newTag: v1.0.0
```
...
kubectl rollout status deployment/personal-finance-web
kubectl rollout status deployment/personal-finance-celery-worker
kubectl rollout status deployment/personal-finance-celery-beat
...
spec:
  ...
  tls:
    - hosts:
        - your-domain.com
      secretName: personal-finance-tls
  rules:
...
kubectl logs -f deployment/personal-finance-celery-worker --all-containers=true

# Use stern for multi-pod log tailing
stern personal-finance-web
```
...
kubectl exec -it deployment/personal-finance-web -- \
  nc -zv personal-finance-pg 5432
```
...
kubectl exec -it deployment/personal-finance-web -- \
  nc -zv personal-finance-redis-master 6379
```
...
```bash
# Scale web pods
kubectl scale deployment/personal-finance-web --replicas=5

# Scale celery workers
kubectl scale deployment/personal-finance-celery-worker --replicas=4
```
...
```bash
# Build new version
docker build -t personal-finance:v1.1.0 .
docker tag personal-finance:v1.1.0 <your-registry>/personal-finance:v1.1.0
docker push <your-registry>/personal-finance:v1.1.0

# Update image using kubectl
kubectl set image deployment/personal-finance-web \
  web=<your-registry>/personal-finance:v1.1.0

# Watch rollout
kubectl rollout status deployment/personal-finance-web

# Check rollout history
kubectl rollout history deployment/personal-finance-web
```
...
kubectl rollout undo deployment/personal-finance-web

# Rollback to specific revision
kubectl rollout undo deployment/personal-finance-web --to-revision=2

# Pause rollout (for issues)
kubectl rollout pause deployment/personal-finance-web

# Resume rollout
kubectl rollout resume deployment/personal-finance-web
```
