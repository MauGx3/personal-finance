...
This directory contains Kubernetes manifests for deploying Personal Finance.
...
```bash
helm install personal_finance ./helm/personal_finance
```
...
```bash
# Web application
kubectl logs -f deployment/personal_finance-web

# Celery worker
kubectl logs -f deployment/personal_finance-celery-worker

# PostgreSQL
kubectl logs -f personal_finance-pg-1
```
...
```bash
kubectl port-forward svc/personal_finance-web 8000:8000
```
...
```bash
kubectl get pods | grep personal_finance-pg
```

Check PostgreSQL logs:
```bash
kubectl logs personal_finance-pg-1
```
