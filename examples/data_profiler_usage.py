...
def example_basic_validation():
    ...
    try:
        ...
        print(
            f"  Fields: {list(portfolio_data[0].keys()) if portfolio_data else []}"
        )
    except ProfileDataError as e:
        print(f"✗ Validation failed: {e}")
...
def example_service_usage():
    ...
    try:
        ...
        print(
            f"  Amount columns detected: {patterns['potential_amount_columns']}"
        )
        ...
        for finding in sensitive_data:
            print(
                f"    - Column '{finding['column']}': {finding['pattern_type']}"
            )
    ...
    if service.is_available():
        ...
        print(
            "2. DataProfiler not available - install with: pip install dataprofiler"
        )
...
def example_error_handling():
    ...
    try:
        validate_profile_data({1, 2, 3})  # Set is not supported
        print("✗ This should not print")
    except ProfileDataError as e:
        print(f"✓ Correctly handled error: {e}")
...
def example_financial_scenarios():
    ...
    try:
        ...
        if quality["outlier_candidates"]:
            print(
                f"  Outlier candidates found in: {[o['column'] for o in quality['outlier_candidates']]}"
            )
