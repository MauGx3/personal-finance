...
from personal_finance.data_sources.b3_import import B3DocumentImporter, B3ImportResult
...
def load_document(file_path: str) -> str:
    ...
    try:
        ...
        try:
            ...
            print(f"Error: Could not decode file '{file_path}'. Please ensure it's a text file.")
            sys.exit(1)
...
def print_import_result(result: B3ImportResult) -> None:
    ...
    if result.transactions:
        ...
        for i, tx in enumerate(result.transactions, 1):
            print(f"{i:2d}. {tx.date} | {tx.ticker} | {tx.transaction_type.value} | "
    ...
    if result.extract_entries:
        ...
        for i, entry in enumerate(result.extract_entries, 1):
            ...
            print(f"{i:2d}. {entry.date} | {entry.description[:30]:30} | "
...
Examples:
  ...
  %(prog)s --file document.txt --auto-detect --output result.json
    ...
    parser.add_argument(
        '--file', '-f',
        required=True,
        help='Path to the B3 document file'
    )
