...
def stock_analysis(request, symbol):
    ...
    try:
        ...
        response_data = {
            ...
            'financials': _serialize_financials(result.financials) if result.financials else None,
...
def stock_financials(request, symbol):
    ...
    try:
        ...
        response_data = _serialize_financials(financials) if financials else None
...
def stock_technical_analysis(request, symbol):
    ...
    try:
        ...
        analysis_service = StockAnalysisService()
        technical_data = analysis_service._perform_technical_analysis(symbol.upper())
...
def stock_sector_comparison(request, symbol):
    ...
    try:
        ...
        market_service = MarketDataService()
        comparison_data = market_service.compare_with_sector(symbol.upper(), sector)
...
def compare_stocks(request):
    ...
    try:
        ...
        metrics = request.data.get('metrics', ['pe_ratio', 'market_cap', 'revenue_growth', 'overall_score'])
        ...
        for symbol in symbols:
            try:
                ...
                if result.financials and result.financials.financial_ratios:
                    stock_data['pe_ratio'] = result.financials.financial_ratios.pe_ratio
                    stock_data['market_cap'] = result.financials.market_cap
                
                if result.revenue_growth:
                    stock_data['revenue_growth'] = result.revenue_growth.get('1y')
