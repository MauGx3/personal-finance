...
class Command(BaseCommand):
    ...
    def _show_dry_run_info(
        self, demo_type: str, symbols: List[str], days: int
    ):
    ...
    def _demo_portfolio_analysis(self, symbols: List[str]):
        ...
        if self.verbosity >= 1:
            self.stdout.write(
                self.style.HTTP_INFO("📈 Portfolio Analysis Demo")
            )
        ...
        if self.verbosity >= 2:
            self.stdout.write(
                f"Sample holdings created: {len(sample_holdings)} positions"
            )
                ...
                for key, value in metrics.items():
                    if isinstance(value, (int, float)):
                        if "percentage" in key:
                            ...
                            "value" in key or "cost" in key or "return" in key
                        ):
                            formatted_value = f"${value:,.2f}"
                        else:
    ...
    def _demo_technical_analysis(self, symbols: List[str], days: int):
        ...
        if self.verbosity >= 1:
            self.stdout.write(
                self.style.HTTP_INFO("📊 Technical Analysis Demo")
            )
        ...
        if self.verbosity >= 2:
            self.stdout.write(
                f"Sample price data created: {len(sample_data)} days"
            )
        ...
        try:
            ...
            df_with_indicators = (
                polars_processor.calculate_technical_indicators(df_with_ma)
            )
    ...
    def _demo_performance_analysis(self, symbols: List[str], days: int):
        ...
        if self.verbosity >= 1:
            self.stdout.write(
                self.style.HTTP_INFO("⚡ Performance Analysis Demo")
            )
    ...
    def _benchmark_dataframe_operations(self, symbols: List[str], days: int):
        ...
        if self.verbosity >= 2:
            self.stdout.write(
                f"Benchmarking with {len(sample_data)} data points"
            )
    ...
    def _show_available_capabilities(self):
        ...
        if self.verbosity >= 1:
            self.stdout.write(
                "🚀 Available Modern Data Analysis Capabilities:"
            )
            ...
            holdings.append(
                {
                    ...
                    "cost_basis": random.uniform(50, 500)
                    * random.randint(10, 100),
                    "current_value": random.uniform(60, 600)
                    * random.randint(10, 100),
                    "purchase_date": (
                        timezone.now()
            ...
            data.append(
                {
                    "symbol": symbol,
                    "date": (
                        current_date - timedelta(days=days - i - 1)
                    ).isoformat(),
