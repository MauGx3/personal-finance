...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Found {len(assets)} assets to update")

        # Update current prices
        updated_count = self._update_current_prices(
            assets, options["force_update"], options["batch_size"]
        )

        # Update historical data if requested
        if options["historical"]:
            historical_count = self._update_historical_data(
                assets, options["days"], options["batch_size"]
            )
            self.stdout.write(
                self.style.SUCCESS(
    ...
    def _get_assets_to_update(self, symbols: list[str] | None) -> list[Asset]:
        ...
        if symbols:
            ...
            assets = Asset.objects.filter(is_active=True, is_tradeable=True)
    ...
    def _update_current_prices(
        ...
        if not force_update:
            assets_to_update = [
                ...
                for asset in assets
                if (
                    asset.last_price_update is None
                    or asset.last_price_update < cutoff_time
                )
            ]
        else:
        ...
        for i in range(0, len(assets_to_update), batch_size):
            ...
            if self.verbosity >= 2:
                ...
                self.stdout.write(f"Processing batch: {', '.join(symbols)}")
    ...
    def _update_asset_price(self, asset: Asset) -> bool:
        ...
        try:
            ...
            if self.verbosity >= 1:
                self.stdout.write(
                    self.style.ERROR(f"Failed to update {asset.symbol}: {e}")
                )
            return False
    ...
    def _update_historical_data(
        ...
        for i in range(0, len(assets), batch_size):
            ...
            for asset in batch:
                try:
                    if self._update_asset_historical_data(
    ...
    def _update_asset_historical_data(
        self, asset: Asset, start_date, end_date
    ) -> bool:
        ...
        if self.dry_run:
            self.stdout.write(
                f"Would update historical data for {asset.symbol}"
            )
            return True
        ...
        try:
            ...
            logger.error(
                f"Historical data update error for {asset.symbol}: {e}"
            )
            return False
