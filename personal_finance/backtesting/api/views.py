...
class StrategyViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        ).prefetch_related("asset_universe")
    ...
    def performance_summary(self, request, pk=None):
        ...
        completed_backtests = strategy.backtests.filter(
            status="completed", result__isnull=False
        )
        ...
        return Response(
            {
                ...
                "average_return": float(avg_data["avg_return"])
                if avg_data["avg_return"]
                else None,
                "average_sharpe_ratio": float(avg_data["avg_sharpe"])
                if avg_data["avg_sharpe"]
                else None,
                "best_backtest": {
                    "id": best_result.id,
                    "name": best_result.name,
                    "total_return": float(best_result.result.total_return),
                    "sharpe_ratio": float(best_result.result.sharpe_ratio)
                    if best_result.result.sharpe_ratio
                    else None,
                }
                if best_result
                else None,
                "worst_backtest": {
                    ...
                    "sharpe_ratio": float(worst_result.result.sharpe_ratio)
                    if worst_result.result.sharpe_ratio
                    else None,
                }
                if worst_result
                else None,
...
class BacktestViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if strategy.user != self.request.user:
            raise ValidationError(
                "Cannot create backtest for strategy owned by another user"
            )
    ...
    def run(self, request, pk=None):
        ...
        try:
            ...
            logger.error("Backtest execution failed: %s", str(e))
            return Response(
    ...
    def performance_chart(self, request, pk=None):
        ...
        portfolio_values = [
            float(snapshot.total_value) for snapshot in snapshots
        ]
        daily_returns = [
            float(snapshot.daily_return) if snapshot.daily_return else 0.0
            for snapshot in snapshots
        ]
        cumulative_returns = [
            float(snapshot.cumulative_return) for snapshot in snapshots
        ]
        ...
        drawdowns = [
            ...
            for value, max_val in zip(portfolio_series, running_max)
        ]
    ...
    def trades(self, request, pk=None):
        ...
        backtest = self.get_object()
        trades = backtest.trades.select_related("asset").order_by(
            "date", "created_at"
        )
    ...
    def summary(self, request):
        ...
        if completed_backtests.exists():
            ...
            best_backtest = completed_backtests.order_by(
                "-result__total_return"
            ).first()
            worst_backtest = completed_backtests.order_by(
                "result__total_return"
            ).first()
            ...
            if best_backtest:
                summary_data["best_performing_backtest"] = {
                    ...
                    "sharpe_ratio": float(best_backtest.result.sharpe_ratio)
                    if best_backtest.result.sharpe_ratio
                    else None,
                }
            ...
            if worst_backtest:
                summary_data["worst_performing_backtest"] = {
                    ...
                    "sharpe_ratio": float(worst_backtest.result.sharpe_ratio)
                    if worst_backtest.result.sharpe_ratio
                    else None,
                }
...
class BacktestComparisonView(viewsets.GenericViewSet):
    ...
    def compare(self, request):
        ...
        comparison_data = {"backtests": [], "metrics_comparison": {}}
        ...
        for metric in metrics:
            values = [
                bt["metrics"][metric]
                for bt in comparison_data["backtests"]
                if bt["metrics"][metric] is not None
            ]
...
def quick_backtest(request):
    ...
    try:
        ...
        if request.data.get("benchmark_symbol"):
            try:
                benchmark_asset = Asset.objects.get(
                    symbol=request.data["benchmark_symbol"]
                )
        ...
        logger.error("Quick backtest failed: %s", str(e))
        return Response(
