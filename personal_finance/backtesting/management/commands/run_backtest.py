...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        # Backtest selection
        parser.add_argument(
            "--backtest-id", type=int, help="Specific backtest ID to run"
        )
        parser.add_argument(
            "--strategy-id",
            type=int,
    ...
    def _create_and_run_strategy(self, options):
        ...
        existing_symbols = set(
            existing_assets.values_list("symbol", flat=True)
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Created strategy: {strategy.name} (ID: {strategy.id})"
            )
        )
    ...
    def _create_backtest_for_strategy(self, options):
        ...
        if options["strategy_id"]:
            try:
                ...
                raise CommandError(
                    f"Strategy with ID {options['strategy_id']} not found"
                )
            ...
            try:
                ...
                raise CommandError(
                    f"Strategy '{options['strategy_name']}' not found"
                )
    ...
    def _create_and_run_backtest_for_strategy(
        ...
        if options.get("benchmark"):
            try:
                benchmark_asset = Asset.objects.get(
                    symbol=options["benchmark"]
                )
            except Asset.DoesNotExist:
                self.stdout.write(
                    self.style.WARNING(
                        f"Benchmark asset '{options['benchmark']}' not found"
                    )
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Created backtest: {backtest.name} (ID: {backtest.id})"
            )
        )
    ...
    def _run_all_pending_backtests(self, options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Found {pending_backtests.count()} pending backtests"
            )
        )
        ...
        for backtest in pending_backtests:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"Failed to run {backtest.name}: {str(e)}"
                    )
                )
    ...
    def _execute_backtest(self, backtest: Backtest):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Backtest completed in {execution_time:.1f}s"
                )
            )
            self.stdout.write("")
            ...
            self.stdout.write("")
            self.stdout.write("Trading Statistics:")
            ...
            if result.benchmark_return:
                self.stdout.write("")
                self.stdout.write("Benchmark Comparison:")
                self.stdout.write(
