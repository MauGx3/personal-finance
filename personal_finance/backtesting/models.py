...
class Strategy(models.Model):
    ...
    description = models.TextField(
        blank=True, help_text="Strategy description and logic"
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text="Whether this strategy is active"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["user", "name"], name="unique_user_strategy_name"
            )
        ]
...
class Backtest(models.Model):
    ...
    name = models.CharField(max_length=200, help_text="Backtest run name")
    description = models.TextField(
        blank=True, help_text="Backtest description and notes"
    )
    ...
    started_at = models.DateTimeField(
        null=True, blank=True, help_text="When backtest execution started"
    )
    completed_at = models.DateTimeField(
        null=True, blank=True, help_text="When backtest execution completed"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class BacktestResult(models.Model):
    ...
    losing_trades = models.IntegerField(
        default=0, help_text="Number of losing trades"
    )
    ...
    final_portfolio_value = models.DecimalField(
        max_digits=20, decimal_places=2, help_text="Final portfolio value"
    )
    cash_balance = models.DecimalField(
        max_digits=20, decimal_places=2, help_text="Final cash balance"
    )
    ...
    daily_returns = JSONField(
        default=list, help_text="Daily portfolio returns as JSON array"
    )
    portfolio_values = JSONField(
        default=list, help_text="Daily portfolio values as JSON array"
    )
    positions_history = JSONField(
        default=list, help_text="Position changes history as JSON array"
    )
    trade_history = JSONField(
        default=list, help_text="Individual trade records as JSON array"
    )
    ...
    def expectancy(self) -> Optional[Decimal]:
        ...
        if (
            ...
            and self.average_win
            and self.average_loss
        ):
...
class BacktestPortfolioSnapshot(models.Model):
    ...
    total_value = models.DecimalField(
        max_digits=20, decimal_places=2, help_text="Total portfolio value"
    )
    cash_balance = models.DecimalField(
        max_digits=20, decimal_places=2, help_text="Cash balance"
    )
    invested_value = models.DecimalField(
        max_digits=20, decimal_places=2, help_text="Value invested in assets"
    )
...
class BacktestTrade(models.Model):
    ...
    trade_type = models.CharField(
        max_length=10, choices=TRADE_TYPES, help_text="Type of trade"
    )
    date = models.DateField(help_text="Trade execution date")
    ...
    price = models.DecimalField(
        max_digits=15, decimal_places=6, help_text="Execution price per share"
    )
