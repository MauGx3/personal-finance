        ...
        if portfolio.date is None or len(portfolio.positions) == 0:
            ...
            for asset in assets:
                if asset.symbol in self.price_data.columns:
                    current_price = (
                        self.price_data.loc[current_date, asset.symbol]
...
class MovingAverageCrossoverStrategy(BaseStrategy):
    ...
    def generate_signals(
        ...
        for asset in self.strategy.asset_universe.all():
            ...
            if (
                current_date not in short_ma.index
                or current_date not in long_ma.index
            ):
                continue
            ...
            prev_short_ma = (
                short_ma.loc[prev_date]
                if prev_date in short_ma.index
                else None
            )
            prev_long_ma = (
                long_ma.loc[prev_date] if prev_date in long_ma.index else None
            )
            ...
            max_position_value = (
                portfolio.total_value * self.strategy.max_position_size
            )
            ...
            if (
                ...
                if quantity > 0 and portfolio.cash >= quantity * current_price:
                    signal_strength = Decimal(
                        str(
                            (oversold_threshold - current_rsi)
                            / oversold_threshold
                        )
                    )
                    trades.append(
...
class BacktestEngine:
    ...
    def run_backtest(self, backtest: Backtest) -> BacktestResult:
        ...
        try:
            with transaction.atomic():
                ...
                if price_data.empty:
                    raise ValueError(
                        "No price data available for backtest period"
                    )

                # Initialize strategy
                strategy_instance = self._create_strategy_instance(
                    backtest.strategy, price_data
                )

                # Initialize portfolio
                portfolio = PortfolioState(
                    cash=backtest.strategy.initial_capital
                )
                ...
                result = self._calculate_results(
                    backtest, simulation_results, price_data
                )
    ...
    def _create_strategy_instance(
        ...
        if not strategy_class:
            raise ValueError(
                f"Unknown strategy type: {strategy.strategy_type}"
            )
    ...
    def _run_simulation(
        ...
        for i, current_date in enumerate(trading_dates):
            ...
            if strategy.should_rebalance(
                current_date.date(), last_rebalance_date
            ):
                trades = strategy.generate_signals(
                    current_date.date(), portfolio
                )
            ...
            self._apply_risk_management(
                backtest, portfolio, current_date.date(), simulation_results
            )
            ...
            simulation_results["daily_values"].append(
            ...
            if (
                backtest.benchmark_asset
                and backtest.benchmark_asset.symbol in price_data.columns
            ):
                benchmark_price = price_data.loc[
                    current_date, backtest.benchmark_asset.symbol
                ]
                if i == 0:
                    ...
                    benchmark_value = float(
                        backtest.strategy.initial_capital
                    ) * (benchmark_price / initial_benchmark_price)
    ...
    def _update_portfolio_prices(
        ...
        for symbol, position in portfolio.positions.items():
            if (
                symbol in price_data.columns
                and current_date in price_data.index
            ):
            ...
            if portfolio.cash >= total_required:
                ...
                trade_record = BacktestTrade.objects.create(
                    ...
                    position_size_percentage=(
                        trade_value / portfolio_value_before
                    )
                    * 100
    ...
    def _apply_risk_management(
        ...
        if (
            not strategy.stop_loss_percentage
            and not strategy.take_profit_percentage
        ):
            return
        ...
        for symbol, position in portfolio.positions.items():
            ...
            unrealized_pnl_pct = (
                position.unrealized_pnl_percentage / 100
            )  # Convert to decimal

            # Stop loss check
            if (
                strategy.stop_loss_percentage
                and unrealized_pnl_pct <= -strategy.stop_loss_percentage
            ):
            ...
            # Take profit check
                ...
                and unrealized_pnl_pct >= strategy.take_profit_percentage
            ):
    ...
    def _create_portfolio_snapshot(
        ...
        if (
            ...
            if current_date in price_data.index:
                current_benchmark_price = price_data.loc[
                    current_date, backtest.benchmark_asset.symbol
                ]
                first_date = price_data.index[0]
                initial_benchmark_price = price_data.loc[
                    first_date, backtest.benchmark_asset.symbol
                ]

                benchmark_value = float(backtest.strategy.initial_capital) * (
                    current_benchmark_price / initial_benchmark_price
                )
                benchmark_return = (
                    benchmark_value / float(backtest.strategy.initial_capital)
                    - 1
        ...
        cumulative_return = (
            float(portfolio.total_value)
            / float(backtest.strategy.initial_capital)
            - 1
        ) * 100
    ...
    def _calculate_results(
        ...
        if volatility > 0:
            excess_return = annualized_return - (
                self.performance_analytics.risk_free_rate * 100
            )
        ...
        winning_trades = len(
            [
                ...
                and self._calculate_trade_pnl(t, trades) > 0
            ]
        )
        ...
        average_win = (
            np.mean(winning_returns) * 100 if winning_returns else None
        )
        average_loss = (
            np.mean(losing_returns) * 100 if losing_returns else None
        )
        ...
        if benchmark_values:
            ...
            if benchmark_daily_returns:
                ...
                # Beta calculation
                covariance = np.cov(strategy_returns[1:], benchmark_series)[0][
                    1
                ]
                ...
                if beta is not None:
                    alpha = annualized_return - (
                        self.performance_analytics.risk_free_rate * 100
                        + beta * (np.mean(benchmark_series) * 252 * 100)
                    )
                ...
                if tracking_error > 0:
                    information_ratio = (
                        annualized_return
                        - (np.mean(benchmark_series) * 252 * 100)
                    ) / (tracking_error * 100)

        # Total transaction costs
        total_transaction_costs = sum(
            float(trade.transaction_cost + trade.slippage_cost)
            for trade in trades
        )
    ...
    def _calculate_trade_pnl(
        ...
        return float(
            sell_trade.portfolio_value_after
