...
class DataProfilerService:
    ...
    def create_profile(
        ...
        try:
            ...
            self._profiler = self._dp_module.Profiler(
                data=validated_data, **profiler_options
            )
    ...
    def analyze_financial_data(
        ...
        if self._dp_available:
            try:
                analysis_results["basic_profile"] = self.create_profile(
                    financial_data
                )
            except Exception as e:
        ...
        analysis_results["financial_patterns"] = (
            self._analyze_financial_patterns(financial_data)
        )
        analysis_results["data_quality"] = self._analyze_data_quality(
            financial_data
        )

        # Sensitive data detection
        if self.enable_sensitive_data_detection:
            analysis_results["sensitive_data_detected"] = (
                self._detect_sensitive_financial_data(financial_data)
            )
    ...
    def _get_profiler_options(self, **kwargs) -> Dict[str, Any]:
        ...
        if not self.enable_sensitive_data_detection:
            profiler_options["options"] = {"data_labeler.is_enabled": False}
    ...
    def _extract_profile_results(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results = {
                ...
                "column_profiles": self._extract_column_profiles(
                    profile_report
                ),
    ...
    def _extract_data_types(report: Dict[str, Any]) -> Dict[str, str]:
        ...
        try:
            data_stats = report.get("data_stats", [])
            return {
                col_stat.get("column_name", "unknown"): col_stat.get(
                    "data_type", "unknown"
                )
                for col_stat in data_stats
            }
                    ...
                    sensitive_findings.append(
                        {
                            ...
                            "confidence": col_stat.get(
                                "data_label_probability", 0.0
                            ),
                        }
                    )
    ...
    def _analyze_data_quality(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        """
        quality_issues = {
            "missing_data_ratio": df.isnull().sum().sum()
    ...
    def _detect_sensitive_financial_data(
        ...
        for col in df.columns:
            ...
            if any(
                self._looks_like_ssn(str(val)) for val in col_data.head(10)
            ):
                sensitive_patterns.append(
                    {
                        "column": col,
...
def profile_data(data: Any, **options) -> Dict[str, Any]:
    ...
    enable_sensitive_detection = options.pop('enable_sensitive_data_detection', True)
    
    # Create service instance
    service = DataProfilerService(enable_sensitive_data_detection=enable_sensitive_detection)
    ...
    if isinstance(validated_data, pd.DataFrame):
        ...
        result["rows"] = validated_data.shape[0] if validated_data.ndim > 0 else 1
        result["columns"] = validated_data.shape[1] if validated_data.ndim > 1 else 1
    ...
    if isinstance(validated_data, pd.DataFrame):
        try:
            ...
            result["financial_patterns"] = analysis.get("financial_patterns", {})
            ...
            if not result["pii_detected"] and enable_sensitive_detection:
                sensitive_findings = analysis.get("sensitive_data_detected", [])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Profile data using DataProfiler")
    parser.add_argument("--path", required=True, help="Path to data file (CSV, JSON, etc.)")
    parser.add_argument("--no-pii", action="store_true", help="Disable PII detection")
    ...
    try:
        # Profile the data
        options = {
            'enable_sensitive_data_detection': not args.no_pii
        }
