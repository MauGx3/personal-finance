...
def _validate_series(series: pd.Series) -> bool:
    ...
    logger.info("Series validation passed: %d values", len(series))
    return True
...
def _validate_list_data(data: list[Any]) -> bool:
    ...
    if all(not isinstance(item, (dict, list)) for item in data):
        logger.info("Simple list validation passed: %d items", len(data))
        return True
    ...
    logger.info("List validation passed: %d items", len(data))
    return True
...
def _validate_dict_data(data: dict[str, Any]) -> bool:
    ...
    logger.info("Dictionary validation passed: %d keys", len(data))
    return True
