...
from .types import PricePoint, HistoricalSeries, HistoricalPricePoint, CompanyInfo
...
class YFinanceAdapter(BaseDataSourceAdapter):
    ...
    def get_current_price(self, symbol: str) -> Optional[PricePoint]:
        ...
        try:
            ...
            latest_volume = int(hist['Volume'].iloc[-1]) if not hist['Volume'].empty else None
            ...
            raise DataSourceError(f"Failed to fetch current price for {symbol}: {e}")
    ...
    def fetch_historical(
        ...
        try:
            ...
            for idx, row in hist.iterrows():
                data_points.append(HistoricalPricePoint(
                    symbol=symbol,
                    date=idx.date(),
                    ...
                    volume=int(row['Volume']) if not row['Volume'] != row['Volume'] else 0,  # Check for NaN
                    adjusted_close=Decimal(str(row['Close']))  # yfinance typically returns adjusted close
                ))
    ...
    def bulk_get_current(self, symbols: List[str]) -> Dict[str, Optional[PricePoint]]:
        ...
        try:
            ...
            data = yf.download(symbols_str, period="1d", interval="1m", group_by="ticker", progress=False)
            ...
            for symbol in validated_symbols:
                try:
                    ...
                    latest_volume = int(symbol_data['Volume'].iloc[-1]) if not symbol_data['Volume'].empty else None
            ...
            for symbol in validated_symbols:
                try:
                    ...
                    logger.warning(f"Individual fetch also failed for {symbol}: {individual_error}")
                    results[symbol] = None
...
class MockAdapter(BaseDataSourceAdapter):
    ...
    def get_company_info(self, symbol: str) -> Optional[CompanyInfo]:
        ...
        return mock_companies.get(symbol, CompanyInfo(
            symbol=symbol,
