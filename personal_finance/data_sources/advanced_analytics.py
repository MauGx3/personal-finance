...
class AdvancedPortfolioAnalyzer:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Advanced analyzer initialized - bt: {self.bt_available}, "
        ...
        try:
            ...
            if "symbol" in df.columns:
                metrics["diversification_score"] = (
                    self._calculate_diversification_score(df)
                )
    ...
    def _calculate_basic_metrics_fallback(
        self, portfolio_data
    ) -> Dict[str, Any]:
        ...
        if isinstance(portfolio_data, list):
            ...
            total_value = sum(
                item.get("current_value", 0) for item in portfolio_data
            )
            total_cost = sum(
                item.get("cost_basis", 0) for item in portfolio_data
            )
            total_return = total_value - total_cost
            return_pct = (
                (total_return / total_cost * 100) if total_cost > 0 else 0
            ...
            for _, row in df.iterrows():
                ...
                if sector:
                    ...
                    sector_allocation[sector] = (
                        sector_allocation.get(sector, 0) + weight
                    )
    ...
    def search_securities(
        self, search_term: str, asset_class: str = "equities"
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if asset_class == "equities":
                ...
                filtered_results = {
                    ...
                    or search_term.lower() in v.get("long_name", "").lower()
                }
            ...
            securities = []
            for symbol, data in list(filtered_results.items())[
                :10
            ]:  # Limit results
                securities.append(
                    {
    ...
    def _search_securities_fallback(
        ...
        for stock in common_stocks:
            if (
                search_lower in stock["symbol"].lower()
                or search_lower in stock["name"].lower()
            ):
        ...
        try:
            ...
            if "close_price" in df.columns:
                ...
                if "trend_strength" in indicators:
                    ma_short = pl.col("close_price").rolling_mean(
                        window_size=10
                    )
                    ma_long = pl.col("close_price").rolling_mean(
                        window_size=50
                    )
                    enhanced_df = enhanced_df.with_columns(
                        [
                            ((ma_short - ma_long) / ma_long).alias(
                                "trend_strength"
                            )
                        ]
                    )
            ...
            if "close_price" in df.columns:
                ...
                if "momentum" in indicators:
                    enhanced_df["momentum_10d"] = (
                        enhanced_df["close_price"]
                        / enhanced_df["close_price"].shift(10)
                        - 1
                    )
