...
class NotaCorretagemParser(B3DocumentParser):
                ...
                if re.search(
                    ...
                    if all([ticker, tx_type, quantity, price, total]):
                        try:
                            ...
                            errors.append(
                                f"Error parsing transaction {line.strip()}: {e}"
                            )
    ...
    def _extract_fees(self, content: str) -> Dict[str, Decimal]:
        ...
        if brokerage_match:
            fees["brokerage_fee"] = self._parse_decimal(
                brokerage_match.group(1)
            )

        # Extract settlement fee
        settlement_match = re.search(self.PATTERNS["fees"], content)
        if settlement_match:
            fees["settlement_fee"] = self._parse_decimal(
                settlement_match.group(1)
            )
    ...
    def parse(self, content: str) -> B3ImportResult:
        ...
        try:
            ...
            for match in entry_matches:
                date_str, description, credit_str, debit_str, balance_str = (
                    match
                )
        ...
        summary = {
            ...
            "period_start": min(e.date for e in extract_entries)
            if extract_entries
            else None,
            "period_end": max(e.date for e in extract_entries)
            if extract_entries
            else None,
        }
...
class B3DocumentImporter:
    ...
    def detect_document_type(self, content: str) -> Optional[str]:
        ...
        if (
            "nota de corretagem" in content_lower
            or "data pregão" in content_lower
        ):
