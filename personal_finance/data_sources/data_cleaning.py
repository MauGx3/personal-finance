...
class FinancialDataCleaner:
    ...
    def _clean_pandas_dataframe(
        ...
        try:
            ...
            if options.get("validate_financial_precision", True):
                validation_results = self._validate_financial_precision_pandas(
                    cleaned_df
                )
                if validation_results["issues"]:
                    logger.warning(
                        f"Financial precision issues found: {validation_results['issues']}"
                    )
    ...
    def _clean_polars_dataframe(
        self, df: pl.DataFrame, options: Dict[str, Any]
    ) -> pl.DataFrame:
    ...
    def _standardize_column_names_polars(
        ...
        for col in df.columns:
            ...
            clean_name = "".join(
                c if c.isalnum() or c == "_" else "_" for c in clean_name
            )
    ...
    def _remove_currency_symbols_pandas(
            ...
            if df[col].dtype == "object":
                for symbol in currency_symbols:
                    if (
                        df[col]
                        .astype(str)
                        .str.contains(f"\\{symbol}", na=False)
                        .any()
                    ):
                        df[col] = (
                            df[col]
                            .astype(str)
                            .str.replace(f"\\{symbol}", "", regex=True)
                        )
                        logger.info(
                            f"Removed currency symbol '{symbol}' from column '{col}'"
                        )

        return df

    def _remove_currency_symbols_polars(
        self, df: pl.DataFrame
    ) -> pl.DataFrame:
    ...
    def _convert_financial_columns_pandas(
        self, df: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        for col in df.columns:
            if any(
                pattern in col.lower() for pattern in financial_column_patterns
            ):
                try:
                    # Try to convert to numeric, handling any remaining non-numeric characters
                    df[col] = pd.to_numeric(df[col], errors="coerce")
                    logger.info(
                        f"Converted financial column '{col}' to numeric"
                    )
                except Exception as e:
                    logger.warning(
    ...
    def _convert_financial_columns_polars(
        self, df: pl.DataFrame
    ) -> pl.DataFrame:
    ...
    def _validate_financial_precision_pandas(
        self, df: pd.DataFrame
    ) -> Dict[str, Any]:
        ...
        financial_columns = [
            ...
            if any(
                ...
                for pattern in ["price", "amount", "value", "cost", "fee"]
            )
        ]
    ...
    def generate_cleaning_report(
        ...
        cleaned_df: Union[pd.DataFrame, pl.DataFrame],
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(original_df, pd.DataFrame) and isinstance(
            ...
            elif isinstance(original_df, pl.DataFrame) and isinstance(
