...
class FinancialDataProfiler:
    ...
    def _prepare_data_for_profiling(
        self,
        data: Union[str, pd.DataFrame, pl.DataFrame, List[Dict]],
        data_type: str,
    ) -> pd.DataFrame:
        ...
        if isinstance(data, str):
            ...
            if self.pandas_available:
                ...
                raise ValueError(
                    "Pandas required for list-to-DataFrame conversion"
                )
        else:
    ...
    def _extract_profiling_results(self, report: Dict) -> Dict[str, Any]:
        ...
        try:
                ...
                for column_name, column_stats in report["data_stats"].items():
                    if isinstance(column_stats, dict):
                        results["column_profiles"][column_name] = {
                            "data_type": column_stats.get(
                                "data_type", "unknown"
                            ),
                            "null_count": column_stats.get("null_count", 0),
                            "unique_count": column_stats.get(
                                "unique_count", 0
                            ),
                            "statistics": column_stats.get("statistics", {}),
                            "data_label": column_stats.get(
                                "data_label", "unknown"
                            ),
                        }
            ...
            results["data_quality_issues"] = (
                self._identify_data_quality_issues(results["column_profiles"])
            )

            # Generate recommendations
            results["recommendations"] = self._generate_recommendations(
                results["column_profiles"], results["sensitive_data_alerts"]
            )
    ...
    def _analyze_financial_patterns(
        ...
        try:
            for column in data.columns:
                ...
                if col_data.dtype == "object":
                    ...
                    if any(
                        any(symbol in val for symbol in ["$", "€", "£", "¥"])
                        for val in sample_values
                    ):
                        financial_analysis["potential_currencies"].append(
                            column
                        )
                ...
                if col_data.dtype == "float64" and column in [
                    c["column"]
                    for c in financial_analysis["financial_columns"]
                ]:
                    # Check for values that might have floating-point precision issues
                    decimal_places = col_data.apply(
    ...
    def _generate_recommendations(
        self, column_profiles: Dict, sensitive_alerts: List[Dict]
    ) -> List[str]:
        ...
        financial_columns = [
            ...
            if any(
                term in col.lower()
                for term in ["price", "amount", "value", "cost"]
            )
        ]
    ...
    def validate_financial_data_types(
