...
class EnhancedPortfolioVisualizer:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Enhanced visualizer initialized - "
            f"plotly: {self.plotly_available}, "
            f"matplotlib: {self.matplotlib_available}"
        )

    def create_advanced_portfolio_dashboard(
        self, portfolio_data: List[Dict], metrics: Dict[str, Any]
    ) -> Optional[str]:
        ...
        try:
            if not self.plotly_available:
                return self._create_basic_dashboard_fallback(
            ...
            if metrics:
                ...
                fig.add_trace(
                    go.Bar(
                        x=metric_names, y=metric_values, name="Performance"
                    ),
                    row=1,
                    col=2,
                )
            ...
            if "risk_metrics" in metrics and portfolio_data:
                ...
                for item in portfolio_data:
                    ...
                    return_val = (
                        (current_val - cost_basis) / cost_basis
                        if cost_basis > 0
                        else 0
                    )
            ...
            return fig.to_html(
                include_plotlyjs="cdn", div_id="portfolio-dashboard"
            )
    ...
    def create_technical_analysis_chart(
        ...
        try:
            ...
            return fig.to_html(
                include_plotlyjs="cdn", div_id="technical-chart"
            )
        ...
        try:
            ...
            for category, data in performance_data.items():
                if isinstance(data, dict):
                    ...
                    value = data.get(
                        "return_percentage", data.get("total_return", 0)
                    )
                else:
                    value = data
                values.append(value)
            ...
            return fig.to_html(
                include_plotlyjs="cdn", div_id="performance-chart"
            )
    ...
    def create_risk_return_scatter(
        self, portfolio_data: List[Dict], benchmark_data: Optional[Dict] = None
    ) -> Optional[str]:
        ...
        try:
            ...
            if symbols:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        marker=dict(
                            size=(
                                [v / max(values) * 50 + 10 for v in values]
                                if values and max(values) > 0
                                else [20 for _ in values]
                            )
                            if values
                            else 20,
                            sizemode="diameter",
                ...
                if benchmark_data:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            text=["Benchmark"],
                            marker=dict(
                                size=20, color="black", symbol="diamond"
                            ),
                            textposition="top center",
                            name="Benchmark",
                        )
            ...
            return fig.to_html(
                include_plotlyjs="cdn", div_id="risk-return-chart"
            )
    ...
    def _create_basic_dashboard_fallback(
        self, portfolio_data: List[Dict], metrics: Dict[str, Any]
    ) -> str:
