...
class BancoInterDocumentParser(ABC):
    ...
    def _parse_decimal(self, value: str, default: Decimal = Decimal('0')) -> Decimal:
        ...
        try:
            ...
            self.logger.warning(f"Failed to parse decimal value '{value}': {e}")
            return default
    
    def _parse_date(self, value: str, formats: List[str] = None) -> Optional[datetime]:
    ...
    def _get_or_create_asset(self, symbol: str, name: str = '', asset_type: str = Asset.ASSET_STOCK) -> Asset:
...
class BancoInterMonthlyReportParser(BancoInterDocumentParser):
    ...
    def validate_format(self) -> bool:
        ...
        try:
            ...
            return any(pattern in ' '.join(headers) for pattern in required_patterns)
    ...
    def _parse_position_row(self, row: pd.Series) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for col in row.index:
                if any(keyword in col.lower() for keyword in ['ativo', 'código', 'papel']):
            ...
            for col in row.index:
                ...
                if 'posição' in col_lower or 'quantidade' in col_lower:
                    ...
                    position_data['return_percentage'] = self._parse_decimal(row[col])
...
class BancoInterBrokerageNoteParser(BancoInterDocumentParser):
    ...
    def validate_format(self) -> bool:
        ...
        try:
            if self.file_path.suffix.lower() not in ['.pdf', '.csv', '.xlsx', '.xls']:
                return False
    ...
    def _parse_transaction_row(self, row: pd.Series) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for col in row.index:
                col_lower = col.lower()
                if any(keyword in col_lower for keyword in ['papel', 'ativo', 'código']):
                    transaction_data['symbol'] = str(row[col]).strip()
                elif 'tipo' in col_lower or 'operação' in col_lower:
                    op_type = str(row[col]).lower()
                    transaction_data['transaction_type'] = 'SELL' if 'venda' in op_type else 'BUY'
                elif 'quantidade' in col_lower:
                    transaction_data['quantity'] = self._parse_decimal(row[col])
...
class BancoInterExtractParser(BancoInterDocumentParser):
    ...
    def validate_format(self) -> bool:
        ...
        try:
            if self.file_path.suffix.lower() not in ['.csv', '.xlsx', '.xls', '.pdf']:
                return False
            ...
            return sum(1 for pattern in patterns if any(pattern in header for header in headers)) >= 2
...
class BancoInterImportService:
    ...
    def import_document(self, file_path: str, document_type: str, user: User) -> DocumentImport:
        ...
        try:
            ...
            if not parser.validate_format():
                raise ImportError(f"Invalid document format for {document_type}")
            
            # Parse document
            parsed_data = parser.parse()
            
            # Import data into database
            imported_count = self._import_parsed_data(parsed_data, user, import_record)
            ...
            self.logger.info(f"Successfully imported {imported_count} transactions from {file_path}")
    ...
    def _get_parser(self, document_type: str, file_path: str, user: User) -> BancoInterDocumentParser:
    ...
    def _import_parsed_data(self, parsed_data: Dict[str, Any], user: User, import_record: DocumentImport) -> int:
        ...
        if 'transactions' in parsed_data:
            for transaction_data in parsed_data['transactions']:
                try:
                    # Get or create asset
                    asset = self._get_or_create_asset(transaction_data['symbol'])
                    ...
                    self.logger.warning(f"Failed to import transaction {transaction_data}: {e}")
                ...
                try:
                    ...
                    position, created = Position.objects.update_or_create(
                        ...
                        defaults={
                            ...
                            'average_cost': Decimal('0'),  # Would need to calculate from transactions
                    ...
                    self.logger.warning(f"Failed to import position {position_data}: {e}")
