...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("🚀 Enhanced Data Analysis Demo"))
    ...
    def _run_comprehensive_demo(
        self, symbols: List[str], days: int, output_format: str
    ):
        """Run comprehensive demo showcasing all features."""
        self.stdout.write(
            self.style.HTTP_INFO("📊 Comprehensive Data Analysis Demo")
        )
        ...
        if "recommendations" in profile_results:
            self.stdout.write(
                f"   📋 {len(profile_results['recommendations'])} recommendations"
            )

        # 3. Advanced Portfolio Analytics
        self.stdout.write("\n3️⃣ Advanced Portfolio Analytics")
        portfolio_data = self._create_portfolio_sample_data(symbols)
        advanced_metrics = advanced_analyzer.calculate_advanced_metrics(
            portfolio_data
        )
        ...
        if hasattr(enhanced_data, "columns"):
            ...
            self.stdout.write(
                f"   📈 Added indicators: {', '.join(new_columns)}"
            )
        ...
        self.stdout.write(
            f"   📅 Exchange: {calendar_info.get('exchange', 'N/A')}"
        )
        self.stdout.write(
            f"   📊 Trading days this month: {calendar_info.get('trading_days_count', 'N/A')}"
        )
        ...
        self.stdout.write(
            f"   🔍 Found {len(search_results)} results for 'apple'"
        )
    ...
    def _run_portfolio_demo(self, symbols: List[str], output_format: str):
        ...
        if "risk_metrics" in metrics:
            ...
            self.stdout.write(
                f"   Max position weight: {risk.get('max_position_weight', 0):.1%}"
            )
            self.stdout.write(
                f"   Number of positions: {risk.get('number_of_positions', 0)}"
            )
            concentration = risk.get("concentration_risk", 0)
            if concentration > 0:
                self.stdout.write(
                    f"   ⚠️  Concentration risk detected: {concentration:.1%}"
                )

    def _run_market_demo(
        self, symbols: List[str], days: int, output_format: str
    ):
        """Run market data demo."""
        self.stdout.write(
            self.style.HTTP_INFO("📈 Market Data Enhancement Demo")
        )
        ...
        for i, indicators in enumerate(indicators_sets, 1):
            ...
            if hasattr(enhanced_data, "columns"):
                new_cols = [
                    col
                    for col in enhanced_data.columns
                    if col.endswith(("_20d", "_10d", "_strength"))
                ]
                self.stdout.write(
                    f"   ✅ Added columns: {', '.join(new_cols)}"
                )
            else:
                self.stdout.write(
                    f"   ✅ Enhanced {len(enhanced_data)} records"
                )
    ...
    def _run_cleaning_demo(self, output_format: str):
        ...
        scenarios = [
            ...
            },
            {
                "name": "Mixed Formats",
                "data": [
                    {"Stock Symbol": "AAPL", "Purchase-Date": "2024/01/01"}
                ],
            },
        ]
        ...
        for i, scenario in enumerate(scenarios, 1):
            ...
            if output_format == "detailed":
                ...
                self.stdout.write(
                    f"   ✅ Successfully cleaned messy {scenario['name'].lower()}"
                )
    ...
    def _run_performance_demo(
        ...
        self.stdout.write(
            self.style.HTTP_INFO("⚡ Performance Comparison Demo")
        )
        ...
        for scenario in test_scenarios:
            ...
            self.stdout.write(
                f"   Speedup: {results[scenario]['speedup']:.2f}x"
            )
    ...
    def _display_portfolio_metrics(
        self, metrics: Dict[str, Any], output_format: str
    ):
        ...
        if output_format == "summary":
            ...
            self.stdout.write(
                f"   📈 Return %: {metrics.get('return_percentage', 0):.2f}%"
            )
            self.stdout.write(
                f"   🏢 Holdings: {metrics.get('holdings_count', 0)}"
            )
