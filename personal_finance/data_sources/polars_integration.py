...
class PolarsDataProcessor:
    ...
    def _create_empty_price_dataframe(
        self,
    ) -> Union[pl.DataFrame, pd.DataFrame]:
    ...
    def _calculate_ma_polars(
        self, df: pl.DataFrame, price_column: str, windows: List[int]
    ) -> pl.DataFrame:
    ...
    def _calculate_ma_pandas(
        ...
        for window in windows:
            ma_column = f"ma_{window}"
            result_df[ma_column] = (
                result_df[price_column].rolling(window=window).mean()
            )
    ...
    def _calculate_indicators_polars(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        lazy_df = lazy_df.with_columns(
            [
                ...
                (pl.col("close_price") - pl.col("close_price").shift(1)).alias(
                    "price_change"
                ),
            ]
        )
                ...
                (
                    ...
                    - (
                        ...
                        / (
                            1
                            + (
                                pl.col("gain").rolling_mean(14)
                                / pl.col("loss").rolling_mean(14)
                            )
                        )
                    )
                ).alias("rsi_14")
    ...
    def _calculate_indicators_pandas(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Calculate RSI
        gain = result_df["price_change"].where(
            result_df["price_change"] > 0, 0
        )
        loss = -result_df["price_change"].where(
            result_df["price_change"] < 0, 0
        )
        ...
        # Calculate Bollinger Bands
        result_df["bb_middle"] = (
            result_df["close_price"].rolling(window=20).mean()
        )
        result_df["bb_std"] = result_df["close_price"].rolling(window=20).std()
        result_df["bb_upper"] = (
            result_df["bb_middle"] + 2 * result_df["bb_std"]
        )
        result_df["bb_lower"] = (
            result_df["bb_middle"] - 2 * result_df["bb_std"]
        )
    ...
    def calculate_advanced_technical_indicators(
        ...
        try:
            if self.polars_available and isinstance(df, pl.DataFrame):
                return self._calculate_advanced_indicators_polars(
                    df, indicators
                )
            elif self.pandas_available and isinstance(df, pd.DataFrame):
                return self._calculate_advanced_indicators_pandas(
                    df, indicators
                )
            else:
                return df
            ...
            logger.error(
                f"Error calculating advanced technical indicators: {e}"
            )
            return df

    def _calculate_advanced_indicators_polars(
        ...
        try:
            ...
            if "atr" in indicators and all(
                col in df.columns for col in ["high_price", "low_price"]
            ):
                high_low = pl.col("high_price") - pl.col("low_price")
                high_close = (
                    pl.col("high_price") - pl.col("close_price").shift(1)
                ).abs()
                low_close = (
                    pl.col("low_price") - pl.col("close_price").shift(1)
                ).abs()
    ...
    def _calculate_advanced_indicators_pandas(
        ...
        try:
            ...
            if "atr" in indicators and all(
                ...
                high_close = (
                    result_df["high_price"] - result_df["close_price"].shift(1)
                ).abs()
                low_close = (
                    result_df["low_price"] - result_df["close_price"].shift(1)
                ).abs()

                result_df["true_range"] = pd.concat(
                    [high_low, high_close, low_close], axis=1
                ).max(axis=1)
                result_df["atr_14"] = (
                    result_df["true_range"].rolling(14).mean()
                )
            ...
            if "williams_r" in indicators and all(
                ...
                result_df["williams_r_14"] = (
                    (highest_high - result_df["close_price"])
                    / (highest_high - lowest_low)
                    * -100
                )
    ...
    def calculate_portfolio_optimization_metrics(
        self, portfolio_data: List[Dict], risk_free_rate: float = 0.02
    ) -> Dict[str, Any]:
        ...
        try:
                ...
                if weights and returns:
                    ...
                    volatility = (
                        abs(
                            sum(
                                (r - avg_return) ** 2 * w
                                for r, w in zip(returns, weights)
                            )
                        )
                        ** 0.5
                    )
                    ...
                    # Sharpe ratio
                    sharpe_ratio = (
                        (avg_return - risk_free_rate) / volatility
                        if volatility > 0
                        else 0
                    )
                    ...
                    optimization_metrics.update(
                        {
                            ...
                            "weight_concentration": max(weights)
                            if weights
                            else 0,
                        }
                    )
            ...
            logger.error(
                f"Error calculating portfolio optimization metrics: {e}"
            )
            return self.calculate_portfolio_metrics(portfolio_data)

    def calculate_portfolio_metrics(
        self, holdings_data: List[Dict]
    ) -> Dict[str, Any]:
    ...
    def _calculate_portfolio_metrics_polars(
        ...
        for col in required_cols:
            if col not in df.columns:
                logger.warning(
                    f"Required column '{col}' not found in holdings data"
                )
                return {}
    ...
    def _calculate_portfolio_metrics_pandas(
