...
from personal_finance.data_sources.types import PricePoint, HistoricalSeries, CompanyInfo
...
class TestYFinanceAdapter:
    ...
    def test_get_current_price_success(self, mock_ticker_class):
        ...
        mock_hist.index[-1].to_pydatetime.return_value = datetime(2023, 1, 1, 12, 0, 0)
    ...
    def test_get_current_price_no_yfinance(self):
        ...
        with patch.dict('sys.modules', {'yfinance': None}):
            with pytest.raises(DataSourceError, match="yfinance library not installed"):
    ...
    def test_fetch_historical_success(self, mock_ticker_class):
        ...
        mock_hist.iterrows.return_value = [
            ...
            (MagicMock(date=lambda: date(2023, 1, 2)), {
                'Open': 103.0, 'High': 108.0, 'Low': 101.0,
                'Close': 106.0, 'Volume': 1200000
            })
        ]
        mock_hist.index = [
            MagicMock(date=lambda: date(2023, 1, 1)),
    ...
    def test_bulk_get_current_success(self, mock_ticker_class):
        ...
        with patch('yfinance.download') as mock_download:
            ...
            for symbol, data in mock_data.items():
                data.empty = False
                data['Close'].iloc.__getitem__.return_value = 150.0 if symbol == 'AAPL' else 300.0
                data['Volume'].iloc.__getitem__.return_value = 1000000
                data.index = [MagicMock()]
                data.index[-1].to_pydatetime.return_value = datetime(2023, 1, 1, 12, 0, 0)
