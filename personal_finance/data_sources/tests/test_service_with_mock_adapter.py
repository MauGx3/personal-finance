...
class TestDataSourceService:
    ...
    def test_fetch_historical_with_parameters(self):
        ...
        historical = self.service.fetch_historical(
    ...
    def test_fetch_historical_caching(self, mock_cache):
        ...
        assert (
            args[2] == self.service.cache_timeout * 12
        )  # 12x timeout for historical
    ...
    def test_get_company_info_caching(self, mock_cache):
        ...
        args, kwargs = mock_cache.set.call_args
        assert (
            args[2] == self.service.cache_timeout * 48
        )  # 48x timeout for company info
...
class TestDataSourceServiceErrorHandling:
    ...
    def test_adapter_error_propagation(self):
        ...
        self.mock_adapter.get_current_price.side_effect = Exception(
            "Network error"
        )

        with pytest.raises(
            DataSourceError, match="Failed to get current price"
        ):
            self.service.get_current_price("AAPL")
    ...
    def test_bulk_fallback_on_error(self):
        ...
        self.mock_adapter.bulk_get_current.side_effect = Exception(
            "Bulk failed"
        )
    ...
    def test_bulk_individual_fallback_partial_failure(self):
        ...
        self.mock_adapter.get_current_price.side_effect = (
            individual_side_effect
        )
