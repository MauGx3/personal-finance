...
from personal_finance.data_sources.adapter import YFinanceAdapter, DataSourceError
from personal_finance.data_sources.services import DataSourceService, create_yfinance_service
    ...
    return {
        "AAPL": {
            ...
            "historical_data": [
                {
                    "date": "2023-01-01",
                    "Open": 148.0, "High": 152.0, "Low": 147.0, "Close": 150.50,
                    "Volume": 45000000
                },
                {
                    "date": "2023-01-02", 
                    "Open": 150.50, "High": 153.0, "Low": 149.0, "Close": 152.0,
                    "Volume": 42000000
                }
            ],
            "company_info": {
...
class TestYFinanceAdapterIntegration:
    ...
    def test_get_current_price_with_fixtures(self, mock_ticker_class, mock_yfinance_data):
        ...
        mock_close.iloc.__getitem__.return_value = mock_yfinance_data["AAPL"]["current_price"]["Close"]
        
        mock_volume = MagicMock()
        mock_volume.iloc = MagicMock()
        mock_volume.iloc.__getitem__.return_value = mock_yfinance_data["AAPL"]["current_price"]["Volume"]
        ...
        mock_timestamp.to_pydatetime.return_value = datetime(2023, 1, 1, 16, 0, 0)
    ...
    @patch('yfinance.Ticker')
    def test_fetch_historical_with_fixtures(self, mock_ticker_class, mock_yfinance_data):
    ...
    def test_bulk_get_current_with_fixtures(self, mock_download, mock_yfinance_data):
        ...
        mock_aapl_data.index[-1].to_pydatetime.return_value = datetime(2023, 1, 1, 16, 0, 0)
        ...
        mock_msft_data.index[-1].to_pydatetime.return_value = datetime(2023, 1, 1, 16, 0, 0)
    ...
    @patch('yfinance.Ticker')
    def test_get_company_info_with_fixtures(self, mock_ticker_class, mock_yfinance_data):
...
class TestDataSourceServiceIntegration:
    ...
    @patch('yfinance.Ticker')
    def test_service_current_price_integration(self, mock_ticker_class, mock_yfinance_data):
