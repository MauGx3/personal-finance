...
class TestWebSocketPriceEndpoint:
    ...
    async def test_handle_ping_message(self):
        ...
        ping_message = json.dumps(
            {"type": "ping", "data": {"timestamp": "2024-01-01T00:00:00"}}
        )

        await endpoint._handle_message(
            "test_conn", mock_websocket, ping_message
        )
    ...
    async def test_handle_invalid_json(self):
        ...
        await endpoint._handle_message(
            "test_conn", mock_websocket, invalid_json
        )
    ...
    async def test_handle_unknown_message_type(self):
        ...
        unknown_message = json.dumps({"type": "unknown_type", "data": {}})

        await endpoint._handle_message(
            "test_conn", mock_websocket, unknown_message
        )
    ...
    async def test_send_price_update(self):
        ...
        price_point = PricePoint(
            symbol="AAPL", price=Decimal("150.25"), change=Decimal("2.50")
        )
