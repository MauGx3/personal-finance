...
class RealtimeDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        popular_assets = Asset.objects.filter(is_active=True).order_by(
            "-current_price"
        )[:20]
...
def realtime_status(request):
    ...
    user_stats = {
        ...
        "subscriptions": {"portfolios": set(), "assets": set()},
    }
    ...
    for connection_id in user_connections:
        if connection_id in connection_manager.connections:
            ...
            user_stats["subscriptions"]["portfolios"].update(
                conn_info["portfolios"]
            )
            user_stats["subscriptions"]["assets"].update(conn_info["assets"])

    # Convert sets to lists for JSON serialization
    user_stats["subscriptions"]["portfolios"] = list(
        user_stats["subscriptions"]["portfolios"]
    )
    user_stats["subscriptions"]["assets"] = list(
        user_stats["subscriptions"]["assets"]
    )

    return JsonResponse(
        {
            "service_status": "running"
            if price_feed_service.is_running
            else "stopped",
            "global_stats": stats,
