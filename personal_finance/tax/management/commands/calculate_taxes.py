...
class Command(BaseCommand):
        ...
        try:
            if options["transaction_id"]:
                self._process_single_transaction(
                    tax_service, options["transaction_id"]
                )
            else:
    ...
    def _process_single_transaction(
        self, tax_service: TaxCalculationService, transaction_id: int
    ):
        ...
        try:
            ...
            self.stdout.write(
                f"Processing transaction {transaction_id}: {trans}"
            )
    ...
    def _process_transactions(
        ...
        if self.dry_run:
            ...
            dividend_count = queryset.filter(
                transaction_type="dividend"
            ).count()

            self.stdout.write("Would process:")
            self.stdout.write(f"  - {buy_count} buy transactions (tax lots)")
            self.stdout.write(
                f"  - {sell_count} sell transactions (capital gains/losses)"
            )
                ...
                for trans in batch:
                    try:
                        ...
                        if not reprocess and self._is_transaction_processed(
                            ...
                            if self.verbose:
                                self.stdout.write(
                                    f"Skipping already processed: {trans}"
                                )
                            continue
                        ...
                        error_count += 1
                        logger.error(
                            "Error processing transaction {trans.id}: %s",
                            str(e),
                        )
                        self.stdout.write(
                            self.style.ERROR(
        ...
        if error_count > 0:
            self.stdout.write(
                self.style.WARNING(
                    "Some transactions had errors. Check logs for details."
                )
            )
    ...
    def _is_transaction_processed(self, trans: Transaction) -> bool:
        ...
        """
        from personal_finance.tax.models import (
            TaxLot,
            CapitalGainLoss,
            DividendIncome,
        )
