...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--report-type',
            choices=['schedule_d', 'form_1099_div', 'form_8949', 'tax_summary', 'loss_carryforward', 'all'],
            default='all',
    ...
    def _generate_reports_for_user(
        ...
        try:
            ...
            self.stdout.write(f"Generating {report_type} reports for user: {username}")
            
            if self.dry_run:
                self.stdout.write(self.style.WARNING("DRY RUN - No reports will be created"))
                self._show_report_preview(user, tax_year, report_type)
                return
            
            reports = self._generate_reports(report_service, user, tax_year, report_type)
    ...
    def _generate_reports_for_all_users(
        ...
        self.stdout.write(f"Generating {report_type} reports for {total_users} users")
        ...
        for i, user in enumerate(users, 1):
            try:
                ...
                error_count += 1
                logger.error(f"Error generating reports for user {user.username}: {str(e)}")
                self.stdout.write(
    ...
    def _generate_reports(
        ...
        if report_type == 'all':
            ...
            reports['schedule_d'] = report_service.generate_schedule_d_report(user, tax_year)
        elif report_type == 'form_1099_div':
            reports['form_1099_div'] = report_service.generate_dividend_report(user, tax_year)
        elif report_type == 'form_8949':
            reports['form_8949'] = report_service.generate_form_8949_report(user, tax_year)
        elif report_type == 'tax_summary':
            reports['tax_summary'] = report_service.generate_tax_summary_report(user, tax_year)
        elif report_type == 'loss_carryforward':
            reports['loss_carryforward'] = report_service.generate_loss_carryforward_report(user, tax_year)
    ...
    def _show_report_preview(self, user: User, tax_year: TaxYear, report_type: str):
        ...
        self.stdout.write(f"  Transactions in {tax_year.year}: {transaction_count}")
        self.stdout.write(f"  Capital gains/losses: {capital_gains_count}")
        self.stdout.write(f"  Dividend payments: {dividend_count}")
        
        if report_type == 'all':
            self.stdout.write(f"  Would generate: 5 report types")
        else:
    ...
    def _display_report_summary(self, reports: dict, username: str):
        ...
        for report_type, report in reports.items():
            self.stdout.write(f"  {report_type}: {report.get_report_type_display()}")
            
            # Show key metrics
            if hasattr(report, 'net_capital_gain_loss') and report.net_capital_gain_loss:
                gain_loss = "gain" if report.net_capital_gain_loss > 0 else "loss"
                self.stdout.write(f"    Net capital {gain_loss}: ${abs(report.net_capital_gain_loss):,.2f}")
            
            if hasattr(report, 'total_dividend_income') and report.total_dividend_income:
                self.stdout.write(f"    Total dividends: ${report.total_dividend_income:,.2f}")
    ...
    def _save_report_files(self, reports: dict, user: User, tax_year: TaxYear):
        ...
        output_path = Path(self.output_dir)
        user_dir = output_path / f"{user.username}_{tax_year.year}"
