...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            logger.error(
                f"Error identifying loss harvesting opportunities: {str(e)}"
            )
    ...
    def _analyze_user(
        ...
        try:
            ...
            self.stdout.write(
                f"Analyzing tax loss harvesting for user: {username}"
            )
            ...
            if generate_recommendations and opportunities and not self.dry_run:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        "Generated recommendations for opportunities"
                    )
                )
    ...
    def _analyze_all_users(
        ...
        self.stdout.write(
            f"Analyzing {total_users} users for tax loss harvesting opportunities"
        )
        ...
        for i, user in enumerate(users, 1):
            try:
                ...
                if i % 10 == 0 or i == total_users:
                    self.stdout.write(
                        f"Progress: {i}/{total_users} users analyzed"
                    )
    ...
    def _display_opportunities(self, opportunities, username: str):
        ...
        for opp in opportunities:
            ...
            if opp.wash_sale_end_date:
                self.stdout.write(
                    f"           | Wash sale ends: {opp.wash_sale_end_date}"
                )
