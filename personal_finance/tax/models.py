...
class TaxYear(models.Model):
    ...
    long_term_capital_gains_thresholds = models.JSONField(
        default=dict, help_text="Long-term capital gains tax brackets"
    )
...
class CapitalGainLoss(models.Model):
    ...
    position = models.ForeignKey(
        "portfolios.Position", on_delete=models.CASCADE
    )
    transaction = models.ForeignKey(
        "portfolios.Transaction",
        on_delete=models.CASCADE,
    ...
    sale_price_per_share = models.DecimalField(
        max_digits=15, decimal_places=6, help_text="Sale price per share"
    )
    quantity_sold = models.DecimalField(
        max_digits=15, decimal_places=6, help_text="Quantity of shares sold"
    )
    gross_proceeds = models.DecimalField(
        max_digits=15, decimal_places=2, help_text="Gross proceeds from sale"
    )

    # Cost basis details
    cost_basis_per_share = models.DecimalField(
        max_digits=15, decimal_places=6, help_text="Cost basis per share"
    )
    total_cost_basis = models.DecimalField(
        max_digits=15,
        decimal_places=2,
...
class DividendIncome(models.Model):
    ...
    dividend_type = models.CharField(
        max_length=20,
        choices=DIVIDEND_TYPES,
        help_text="Type of dividend for tax treatment",
    )
    amount_per_share = models.DecimalField(
        max_digits=10, decimal_places=6, help_text="Dividend amount per share"
    )
...
class TaxLossHarvestingOpportunity(models.Model):
    ...
    current_unrealized_loss = models.DecimalField(
        max_digits=15, decimal_places=2, help_text="Current unrealized loss"
    )
    wash_sale_end_date = models.DateField(
        null=True, blank=True, help_text="Date when wash sale period ends"
    )
    ...
    recommended_sale_date = models.DateField(
        null=True, blank=True, help_text="Recommended date to execute sale"
    )
...
class TaxOptimizationRecommendation(models.Model):
    ...
    deadline = models.DateField(
        null=True, blank=True, help_text="Deadline for optimal implementation"
    )
...
class TaxReport(models.Model):
    ...
    report_data = models.JSONField(
        help_text="Complete report data and calculations"
    )
