...
class TaxReportService:
    ...
    def generate_schedule_d_report(
        ...
        """
        logger.info(
            f"Generating Schedule D report for {user.username} - {tax_year.year}"
        )

        # Calculate capital gains/losses
        capital_gains_data = (
            self.tax_calc_service.calculate_capital_gains_losses(
                user, tax_year
            )
        )
        ...
        gains_losses = (
            CapitalGainLoss.objects.filter(user=user, tax_year=tax_year)
            .select_related("position__asset", "transaction")
            .order_by("term", "sale_date", "position__asset__symbol")
        )
        ...
        report_data = {
            ...
            "form_totals": {
                ...
                "net_capital_gain_loss": capital_gains_data["totals"][
        ...
        tax_report, created = TaxReport.objects.update_or_create(
            ...
            defaults={
                ...
                "total_short_term_gains": capital_gains_data["short_term"][
                    "gains"
                ],
                "total_short_term_losses": capital_gains_data["short_term"][
                    "losses"
                ],
                "total_long_term_gains": capital_gains_data["long_term"][
                    "gains"
                ],
                "total_long_term_losses": capital_gains_data["long_term"][
                    "losses"
                ],
                "net_capital_gain_loss": capital_gains_data["totals"][
                    "net_capital_gain_loss"
    ...
    def generate_dividend_report(
        ...
        """
        logger.info(
            f"Generating dividend report for {user.username} - {tax_year.year}"
        )
        ...
        dividends = (
            DividendIncome.objects.filter(user=user, tax_year=tax_year)
            .select_related("position__asset")
            .order_by("position__asset__symbol", "payment_date")
        )
        ...
        for dividend in dividends:
            ...
            if dividend.dividend_type == "qualified":
                ...
                asset_data["capital_gain_distributions"] += (
    ...
    def generate_form_8949_report(
        ...
        """
        logger.info(
            f"Generating Form 8949 report for {user.username} - {tax_year.year}"
        )

        # Get all capital gains/losses with full detail
        gains_losses = (
            CapitalGainLoss.objects.filter(user=user, tax_year=tax_year)
            .select_related("position__asset", "transaction", "tax_lot")
            ...
            transactions.append(
                {
                    ...
                    "symbol": gain_loss.position.asset.symbol,
                    "acquisition_date": gain_loss.tax_lot.acquisition_date.strftime(
                        "%m/%d/%Y"
                    ),
                    ...
                    "cost_basis": gain_loss.total_cost_basis,
                    "adjustment_code": "W"
                    if gain_loss.wash_sale_adjustment != 0
                    else "",
    ...
    def generate_tax_summary_report(
        ...
        """
        logger.info(
            f"Generating tax summary report for {user.username} - {tax_year.year}"
        )
        ...
        optimization_recommendations = (
            TaxOptimizationRecommendation.objects.filter(
                user=user, tax_year=tax_year, is_implemented=False
            ).order_by("-estimated_tax_savings")
        )

        # Calculate estimated tax liability (simplified)
        net_capital_gains = capital_gains_data["totals"][
            "net_capital_gain_loss"
        ]
        ...
        total_estimated_tax = (
            estimated_tax_on_gains + estimated_tax_on_dividends
        )
        ...
        report_data = {
            ...
            "optimization": {
                ...
                "total_potential_losses": sum(
                    opp.potential_loss_amount for opp in loss_opportunities
                ),
    ...
    def generate_loss_carryforward_report(
        ...
        """
        logger.info(
            f"Generating loss carryforward report for {user.username} - {tax_year.year}"
        )
        ...
        for year in range(current_year - 8, current_year):  # Look back 8 years
            try:
                ...
                prev_report = TaxReport.objects.filter(
                    user=user, tax_year=prev_tax_year, report_type="schedule_d"
                ).first()
        ...
        total_carryforward = sum(
            loss["carryforward"] for loss in previous_years_losses
        )
            ...
            "expiration_schedule": [
                {
                    ...
                    "expires": loss["year"]
                    + 8,  # Capital losses carry forward indefinitely (post-1997)
                }
    ...
    def generate_all_tax_reports(
        ...
        """
        logger.info(
            f"Generating all tax reports for {user.username} - {tax_year.year}"
        )

        reports = {}

        try:
            reports["schedule_d"] = self.generate_schedule_d_report(
                user, tax_year
            )
            reports["form_1099_div"] = self.generate_dividend_report(
                user, tax_year
            )
            reports["form_8949"] = self.generate_form_8949_report(
                user, tax_year
            )
            reports["tax_summary"] = self.generate_tax_summary_report(
                user, tax_year
            )
            ...
            logger.info("Generated %s tax reports successfully", len(reports))
