...
class TaxLotSerializer(serializers.ModelSerializer):
    ...
    asset_symbol = serializers.CharField(
        source="position.asset.symbol", read_only=True
    )
    asset_name = serializers.CharField(
        source="position.asset.name", read_only=True
    )
    portfolio_name = serializers.CharField(
        source="position.portfolio.name", read_only=True
    )
...
class CapitalGainLossSerializer(serializers.ModelSerializer):
    ...
    tax_year_display = serializers.CharField(
        source="tax_year.year", read_only=True
    )
    term_display = serializers.CharField(
        source="get_term_display", read_only=True
    )
...
class DividendIncomeSerializer(serializers.ModelSerializer):
    ...
    dividend_type_display = serializers.CharField(
        source="get_dividend_type_display", read_only=True
    )
...
class TaxLossHarvestingOpportunitySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "current_unrealized_loss",
            "wash_sale_end_date",
            "status",
            "status_display",
            "recommended_sale_date",
            "alternative_investments",
            "tax_benefit_estimate",
            "created_at",
            "updated_at",
        ]
        read_only_fields = [
...
class TaxReportSerializer(serializers.ModelSerializer):
    ...
    net_short_term = serializers.DecimalField(
        max_digits=15, decimal_places=2, read_only=True
    )
    net_long_term = serializers.DecimalField(
        max_digits=15, decimal_places=2, read_only=True
    )
...
class TaxCalculationRequestSerializer(serializers.Serializer):
    ...
    year = serializers.IntegerField(
        required=False, help_text="Tax year (current year if not specified)"
    )
    portfolio_id = serializers.IntegerField(
        required=False,
        help_text="Specific portfolio ID (all portfolios if not specified)",
    )
