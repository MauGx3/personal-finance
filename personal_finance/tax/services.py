...
class TaxCalculationService:
    ...
    def _process_sale_transaction(
        ...
        tax_lots = TaxLot.objects.filter(
            position=transaction.position, remaining_quantity__gt=0
        ...
        for tax_lot in tax_lots:
            ...
            capital_gain_loss = CapitalGainLoss.objects.create(
                ...
                + fee_allocation,  # Before fees for reporting
        ...
        for portfolio in portfolios:
            ...
            for position in positions:
                ...
                if unrealized_loss < -minimum_loss_threshold:
                    ...
                    opportunity, created = (
                        TaxLossHarvestingOpportunity.objects.get_or_create(
                            user=user,
                            position=position,
                            tax_year=tax_year,
                            defaults={
                                ...
                                "status": "wash_sale_risk"
                                if wash_sale_end_date
                                else "identified",
                            },
                        )
                    )
                    ...
                    if not created:
                        ...
                        opportunity.potential_loss_amount = abs(
                            unrealized_loss
                        )
    ...
    def generate_loss_harvesting_recommendations(
        ...
        for opportunity in opportunities:
            if opportunity.status == "wash_sale_risk":
                ...
                opportunity.recommended_sale_date = (
                    opportunity.wash_sale_end_date
                )
                opportunity.alternative_investments = (
                    self._suggest_alternative_investments(
...
class TaxOptimizationService:
    ...
    def generate_tax_optimization_recommendations(
        ...
        for portfolio in portfolios:
            recommendations.extend(
                self._analyze_asset_location(user, portfolio, tax_year)
            )
            recommendations.extend(
                self._analyze_rebalancing_opportunities(
                    user, portfolio, tax_year
                )
            )
            recommendations.extend(
                self._analyze_holding_periods(user, portfolio, tax_year)
            )

        # General tax recommendations
        recommendations.extend(
            self._generate_general_tax_recommendations(user, tax_year)
        )
    ...
    def _analyze_rebalancing_opportunities(
        ...
        positions = Position.objects.filter(portfolio=portfolio)
        total_value = sum(
            p.quantity * (p.asset.current_price or p.average_cost)
            for p in positions
        )
    ...
    def _analyze_holding_periods(
        ...
        tax_lots = TaxLot.objects.filter(
            position__portfolio=portfolio, remaining_quantity__gt=0
        )
        ...
        if near_long_term_lots:
            total_potential_savings = sum(
                lot.unrealized_gain_loss
                * Decimal("0.15")  # Assume 15% LTCG rate vs 22% ordinary
            ...
            recommendation = TaxOptimizationRecommendation(
                ...
                deadline=min(
                    lot.acquisition_date + timedelta(days=366)
                    for lot in near_long_term_lots
                ),
            )
            recommendations.append(recommendation)
