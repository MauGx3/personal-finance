...
class TaxLotViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            TaxLot.objects.filter(position__portfolio__user=self.request.user)
            .select_related(
...
class CapitalGainLossViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            CapitalGainLoss.objects.filter(user=self.request.user)
            .select_related(
                "tax_year",
                "position__asset",
                "position__portfolio",
...
class DividendIncomeViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            DividendIncome.objects.filter(user=self.request.user)
            .select_related(
...
class TaxLossHarvestingOpportunityViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            TaxLossHarvestingOpportunity.objects.filter(user=self.request.user)
            .select_related(
    ...
    def analyze(self, request):
        ...
        generate_recommendations = serializer.validated_data[
        ...
        try:
            ...
            opportunities = (
                loss_service.identify_loss_harvesting_opportunities(
                    request.user, minimum_loss
                )
            )

            if generate_recommendations and opportunities:
                loss_service.generate_loss_harvesting_recommendations(
                    opportunities
                )
            ...
            logger.error(
                "Error analyzing loss harvesting opportunities: %s", str(e)
            )
            return Response(
...
class TaxOptimizationRecommendationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .select_related("tax_year", "portfolio")
    ...
    def generate(self, request):
        ...
        try:
            ...
            recommendations = (
                optimization_service.generate_tax_optimization_recommendations(
            ...
            serializer = TaxOptimizationRecommendationSerializer(
            ...
            logger.error(
                "Error generating optimization recommendations: %s", str(e)
            )
            return Response(
...
class TaxReportViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            TaxReport.objects.filter(user=self.request.user)
    ...
    def generate(self, request):
        ...
        try:
            ...
            if report_type == "all":
                reports = report_service.generate_all_tax_reports(
                    request.user, tax_year
                )
            elif report_type == "schedule_d":
                reports = {
                    "schedule_d": report_service.generate_schedule_d_report(
                        request.user, tax_year
                    )
                }
            elif report_type == "form_1099_div":
                reports = {
                    "form_1099_div": report_service.generate_dividend_report(
                        request.user, tax_year
                    )
                }
            elif report_type == "form_8949":
                reports = {
                    "form_8949": report_service.generate_form_8949_report(
                        request.user, tax_year
                    )
                }
            elif report_type == "tax_summary":
                reports = {
                    "tax_summary": report_service.generate_tax_summary_report(
                        request.user, tax_year
                    )
                }
            elif report_type == "loss_carryforward":
                reports = {
                    "loss_carryforward": report_service.generate_loss_carryforward_report(
                        request.user, tax_year
                    )
                }
            else:
    ...
    def summary(self, request):
        ...
        try:
            ...
            loss_opportunities = TaxLossHarvestingOpportunity.objects.filter(
                user=request.user, tax_year=tax_year
            ).count()

            recommendations = TaxOptimizationRecommendation.objects.filter(
                user=request.user, tax_year=tax_year, is_implemented=False
            ).count()
            ...
            summary_data = {
                ...
                "summary_metrics": {
                    ...
                    "taxable_events": capital_gains["transactions_count"]
                    + dividends["payment_count"],
                },
            }
    ...
    def calculate(self, request):
        ...
        for transaction in queryset:
            try:
                ...
                if not reprocess and self._is_transaction_processed(
                    transaction
                ):
                    continue
                ...
                error_count += 1
                logger.error(
                    "Error processing transaction {transaction.id}: %s", str(e)
                )
