...
class PortfolioCharts:
    ...
    def create_portfolio_performance_chart(
        ...
        if PortfolioSnapshot is None:
            ...
            return self._create_empty_chart(
                "Portfolio snapshots not available"
            )

        try:
            # Get portfolio snapshots for the period
            snapshots = PortfolioSnapshot.objects.filter(
                portfolio=portfolio, date__gte=start_date, date__lte=end_date
            ).order_by("date")

            if not snapshots.exists():
                logger.warning(
                    "No snapshot data for portfolio %s", portfolio.id
                )
                return self._create_empty_chart(
                ...
                [
                    {
                        ...
                        "daily_return": float(
                            snapshot.daily_return_percent or 0
                        ),
                    }
            ...
            fig.update_yaxes(
                title_text=f"Portfolio Value ({currency_symbol})", row=1, col=1
            )
        ...
        try:
            ...
            fig = go.Figure(
                data=[
                    go.Pie(
                        labels=[
                            f"{item['symbol']}<br>{item['name']}"
                            for item in allocation_data
                        ],
                        values=[
                            item["percentage"] for item in allocation_data
                        ],
    ...
    def create_risk_metrics_chart(
        ...
        if Portfolio is None or PerformanceAnalytics is None:
            logger.warning(
                "Portfolio model or PerformanceAnalytics not available"
            )
            ...
            fig.add_trace(
                go.Indicator(
                    ...
                    gauge={
                        "axis": {"range": [-2, 3]},
                        "bar": {
                            "color": self._get_risk_color(sharpe, "sharpe")
                        },
                        "steps": [
...
class AssetCharts:
    ...
    def create_price_chart_with_indicators(
        ...
        try:
            ...
            fig.update_yaxes(
                title_text=f"Price ({currency_symbol})", row=1, col=1
            )
        ...
        for indicator in indicators:
            try:
                if indicator.lower() == "sma_20":
                    ...
                    rsi_data = (
                        self.technical_indicators.relative_strength_index(
