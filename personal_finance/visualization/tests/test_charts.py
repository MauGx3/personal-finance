...
class PortfolioChartsTestCase(TestCase):
    ...
    def test_get_risk_color_sharpe_ratio(self):
        ...
        # Good Sharpe ratio (>= 1)
        self.assertEqual(
            self.chart_generator._get_risk_color(1.5, "sharpe"), "green"
        )

        # Moderate Sharpe ratio (0 to 1)
        self.assertEqual(
            self.chart_generator._get_risk_color(0.5, "sharpe"), "yellow"
        )

        # Poor Sharpe ratio (< 0)
        self.assertEqual(
            self.chart_generator._get_risk_color(-0.5, "sharpe"), "red"
        )
    ...
    def test_get_risk_color_drawdown(self):
        ...
        # Low drawdown (<= 10%)
        self.assertEqual(
            self.chart_generator._get_risk_color(5.0, "drawdown"), "green"
        )

        # Moderate drawdown (10-25%)
        self.assertEqual(
            self.chart_generator._get_risk_color(15.0, "drawdown"), "yellow"
        )

        # High drawdown (> 25%)
        self.assertEqual(
            self.chart_generator._get_risk_color(30.0, "drawdown"), "red"
        )
    ...
    def test_create_asset_allocation_chart_no_positions(self, mock_position):
        ...
        result = self.chart_generator.create_asset_allocation_chart(
            self.mock_portfolio, "USD"
        )
