...
class DashboardSummaryAPISecurityTestCase(TestCase):
    ...
    def test_dashboard_summary_api_error_does_not_expose_exception_details(self, mock_portfolio):
        """Test that exception details are not exposed in API response."""
        # Mock Portfolio.objects.filter to raise an exception
        mock_portfolio.objects.filter.side_effect = Exception("Internal database error with sensitive info")
        ...
        self.assertEqual(response_data['error'], 'Failed to generate dashboard summary')
