...
class PortfolioDetailView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs) -> Dict[str, Any]:
        ...
        portfolio = get_object_or_404(
            Portfolio, id=portfolio_id, user=self.request.user
        )

        # Get positions for the portfolio
        positions = portfolio.positions.filter(is_active=True).select_related(
            "asset"
        )
...
@login_required
def portfolio_performance_chart_api(
...
@login_required
def portfolio_allocation_chart_api(
    request: HttpRequest, portfolio_id: int
) -> JsonResponse:
...
@login_required
def portfolio_risk_metrics_chart_api(
    request: HttpRequest, portfolio_id: int
) -> JsonResponse:
