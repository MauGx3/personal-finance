...
class FeatureRegistry:
    ...
    def register_feature(
        ...
        feature_info = FeatureInfo(
            name=name, module_path=module_path, is_available=False
        )
        ...
        try:
            ...
            if required:
                self._logger.error(
                    f"Required feature '{name}' is unavailable: {e}"
                )
                raise ImportError(
                    f"Required feature '{name}' cannot be imported: {e}"
                )
...
def register_optional_feature(
    name: str, module_path: str, component_name: Optional[str] = None
) -> Any:
    """
    ...
    """
    feature_info = registry.register_feature(
        name, module_path, component_name, required=False
    )
...
def register_required_feature(
    ...
    """
    feature_info = registry.register_feature(
        name, module_path, component_name, required=True
    )
