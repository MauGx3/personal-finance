...
def test_b3_data_source_integration():
    ...
    b3_sources = [source for source in manager.sources if source.name == "B3 Brazil"]
...
def test_data_source_manager_with_b3():
    ...
    if price_data:
        print(f"✓ Manager price fetch: {price_data.symbol} = {price_data.current_price} {price_data.currency}")
    else:
        print("⚠️  Manager price fetch returned None (expected if sources are unavailable)")
