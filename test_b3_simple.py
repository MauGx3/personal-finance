...
def test_nota_corretagem_parsing():
    ...
    sell_tx = [tx for tx in result.transactions if tx.transaction_type == B3TransactionType.SELL][0]
...
def test_extrato_parsing():
    ...
    entries_with_dates = [e for e in result.extract_entries if e.date is not None]
...
def test_document_importer():
    ...
    assert detected == "nota_corretagem", f"Should detect nota_corretagem, got {detected}"
...
def test_decimal_parsing():
    ...
    for input_val, expected in test_cases:
        ...
        assert result == expected_decimal, f"Expected {expected_decimal}, got {result} for input {input_val}"
...
def test_real_document_samples():
    ...
    try:
        with open("examples/sample_nota_corretagem.txt", "r", encoding="utf-8") as f:
            content = f.read()
        
        result = importer.import_document(content, "nota_corretagem")
        print(f"  Sample Nota de Corretagem: {len(result.transactions)} transactions")
        assert len(result.transactions) > 0, "Should find transactions in sample document"
    ...
    try:
        ...
        assert len(result.extract_entries) > 0, "Should find entries in sample document"
