...
try:
    ...
    def test_parsers():
        ...
        if Path(monthly_file).exists():
            ...
            try:
                ...
                print(
                    f"   ✓ Parsed {len(parsed_data.get('positions', []))} positions"
                )

                for i, pos in enumerate(parsed_data.get("positions", [])[:3]):
                    print(
                        f"     Position {i + 1}: {pos['symbol']} - {pos['quantity']} shares - R$ {pos['value']}"
                    )
        ...
        if Path(brokerage_file).exists():
            ...
            try:
                parsed_data = parser.parse()
                print(
                    f"   ✓ Parsed {len(parsed_data.get('transactions', []))} transactions"
                )

                for i, tx in enumerate(
                    parsed_data.get("transactions", [])[:3]
                ):
                    print(
                        f"     Transaction {i + 1}: {tx['symbol']} - {tx['transaction_type']} - {tx['quantity']} @ R$ {tx['price']}"
                    )
            except Exception as e:
                print(f"   ❌ Parse error: {e}")
        else:
        ...
        if Path(extract_file).exists():
            ...
            try:
                ...
                for i, tx in enumerate(
                    ...
                    print(
                        f"     Transaction {i + 1}: {tx['description']} - R$ {tx['amount']}"
                    )
            except Exception as e:
                print(f"   ❌ Parse error: {e}")
        else:
    ...
    def test_import_service():
        ...
        if Path(monthly_file).exists():
            try:
                ...
                print(
                    f"   ✓ Imported Transactions: {import_record.imported_transactions_count}"
                )
                print(
                    f"   ✓ Document Type: {import_record.get_document_type_display()}"
                )
        ...
        completed_imports = DocumentImport.objects.filter(
            user=user, status="COMPLETED"
        ).count()
        failed_imports = DocumentImport.objects.filter(
    ...
    def test_models():
        ...
        print(
            f"     - Document Type: {doc_import.get_document_type_display()}"
        )
    ...
    if __name__ == "__main__":
        ...
        print(
            "2. Test the API endpoints using the documentation in BANCO_INTER_IMPORT.md"
        )
    ...
    print(
        "Make sure you have Django installed and the project dependencies available."
    )
except Exception as e:
