...
def test_create_asset():
    asset = Asset.objects.create(symbol="TEST", name="Test Asset", asset_type=Asset.ASSET_STOCK)
...
def test_portfolio_and_holding_relationships():
    ...
    if callable(create_user):
        ...
        kwargs = {'password': 'pass1234', 'email': 'test@example.com'}
    ...
    asset = Asset.objects.create(symbol="AAA", name="Asset AAA", asset_type=Asset.ASSET_STOCK)
    portfolio = Portfolio.objects.create(user=user, name="My Portfolio", is_default=True)

    holding = Holding.objects.create(user=user, asset=asset, portfolio=portfolio, quantity=1)
    ...
    # unique constraint: cannot create duplicate holding for same user/asset/portfolio
    with pytest.raises(IntegrityError):
        Holding.objects.create(user=user, asset=asset, portfolio=portfolio, quantity=2)
