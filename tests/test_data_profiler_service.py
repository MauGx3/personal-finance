...
class TestDataProfilerService:
    ...
    def test_service_initialization_without_dataprofiler():
        ...
        with patch(
            ...
            service = DataProfilerService.__new__(
                DataProfilerService
    ...
    def test_create_profile_success_path():
        ...
        with patch(
            ...
            mock_logger.info.assert_any_call(
                "Data validation successful, creating profile"
            )
    ...
    def test_analyze_financial_data_invalid_input():
        ...
        with pytest.raises(
            ProfileDataError, match="Financial data must be a pandas DataFrame"
        ):
    ...
    def test_analyze_financial_data_dataprofiler_error():
        ...
        with patch.object(service, "create_profile") as mock_create_profile:
            ...
            with patch(
                ...
                mock_logger.warning.assert_called_with(
                    "Basic profiling failed: Profile creation failed"
                )
...
class TestFinancialPatternAnalysis:
    ...
    def test_looks_like_currency_column():
        ...
        # Test positive cases
        assert service._looks_like_currency_column(
            "amount", pd.Series([1, 2, 3])
        )
        assert service._looks_like_currency_column(
            "price", pd.Series([1, 2, 3])
        )
        assert service._looks_like_currency_column(
            "total_cost", pd.Series([1, 2, 3])
        )
        assert service._looks_like_currency_column(
            "account_balance", pd.Series([1, 2, 3])
        )

        # Test negative cases
        assert not service._looks_like_currency_column(
    ...
    def test_looks_like_date_column():
        ...
        # Test positive cases
        assert service._looks_like_date_column(
            "transaction_date", pd.Series(["2024-01-01"])
        )
        assert service._looks_like_date_column(
            "created_at", pd.Series(["2024-01-01"])
        )
        assert service._looks_like_date_column(
            "timestamp", pd.Series(["2024-01-01"])
        )

        # Test negative cases
        assert not service._looks_like_date_column(
            "amount", pd.Series([1, 2, 3])
        )
        assert not service._looks_like_date_column(
            "name", pd.Series(["a", "b", "c"])
    ...
    def test_check_suspicious_patterns():
        ...
        suspicious = service._check_suspicious_patterns(
            "amount", high_value_series
        )
...
class TestSensitiveDataDetection:
    ...
    def test_detect_sensitive_financial_data():
        ...
        ssn_findings = [
            s for s in sensitive_data if s["pattern_type"] == "potential_ssn"
        ]
...
class TestIntegrationScenarios:
    ...
    def test_full_financial_analysis_workflow():
        ...
        service._dp_available = (
            False  # Disable DataProfiler for predictable testing
        )
