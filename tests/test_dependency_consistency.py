...
class TestDependencyConsistency:
    ...
    def test_critical_packages_pinned_in_constraints(self, repo_root):
        ...
        critical_packages = [
            'django', 'pandas', 'numpy', 'yfinance', 'fastapi', 'uvicorn',
            'gunicorn', 'celery', 'redis', 'sqlalchemy', 'alembic'
        ]
        
        for package in critical_packages:
            assert package in constraints, f"Critical package {package} not found in constraints.txt"
            assert constraints[package].startswith('=='), f"Critical package {package} should be pinned with == in constraints.txt"
    ...
    def test_pyproject_has_minimum_versions(self, repo_root):
        ...
        for package in core_packages:
            assert package in dependencies, f"Core package {package} not found in pyproject.toml"
            # Should have some version constraint (not empty)
            if dependencies[package]:
                assert any(op in dependencies[package] for op in ['>=', '>', '==', '<']), \
    ...
    def test_rcssmin_django_compressor_compatibility(self, repo_root):
        ...
        assert 'rcssmin' in dependencies, "rcssmin should be specified in requirements/base.txt"
        assert dependencies['rcssmin'] == '==1.1.2', \
            "rcssmin should be pinned to 1.1.2 for django-compressor compatibility"
        
        assert 'django-compressor' in dependencies, "django-compressor should be specified in requirements/base.txt"
    ...
    def test_constraints_versions_compatible_with_lock(self, repo_root):
        ...
        for package in common_packages:
            ...
            assert constraints_version, f"Package {package} should have version in constraints.txt"
            assert lock_version, f"Package {package} should have version in requirements.lock"
    ...
    def test_requirements_files_structure(self, repo_root):
        ...
        if prod_file.exists():
            with open(prod_file, 'r') as f:
                content = f.read()
            assert '-r base.txt' in content, "requirements/production.txt should include base.txt"
        ...
        if local_file.exists():
            with open(local_file, 'r') as f:
                content = f.read()
            assert '-r production.txt' in content, "requirements/local.txt should include production.txt"
