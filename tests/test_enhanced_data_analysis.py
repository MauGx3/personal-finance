...
try:
    from personal_finance.data_sources.polars_integration import (
        polars_processor,
    )
...
class TestDataAnalysisIntegration:
    ...
    @pytest.mark.skipif(
        not POLARS_INTEGRATION_AVAILABLE,
        reason="Polars integration not available",
    )
    ...
    def test_moving_averages_calculation(self, sample_price_data):
        ...
        df = polars_processor.create_price_dataframe(sample_price_data)
        df_with_ma = polars_processor.calculate_moving_averages(
            df, windows=[2]
        )
    ...
    def test_technical_indicators_calculation(self, sample_price_data):
        ...
        extended_data = (
            sample_price_data * 20
        )  # Repeat data to have enough points
        for i, item in enumerate(extended_data):
            item["date"] = (datetime(2024, 1, 1) + timedelta(days=i)).strftime(
                "%Y-%m-%d"
            )
            item["close_price"] = 180 + (i * 0.5)  # Trending upward

        df = polars_processor.create_price_dataframe(extended_data)
        df_with_indicators = polars_processor.calculate_technical_indicators(
            df
        )
    ...
    def test_portfolio_metrics_calculation(self, sample_portfolio_data):
        ...
        metrics = polars_processor.calculate_portfolio_metrics(
        ...
        expected_total_value = sum(
            item["current_value"] for item in sample_portfolio_data
        )
        expected_total_cost = sum(
            item["cost_basis"] for item in sample_portfolio_data
        )
    ...
    @pytest.mark.skipif(
        not DATA_PROFILING_AVAILABLE, reason="Data profiling not available"
    )
    def test_data_profiling_basic_functionality(self, sample_portfolio_data):
        ...
        if PANDAS_AVAILABLE:
            ...
            profile_results = financial_profiler.profile_financial_data(
                df, "dataframe"
            )
    ...
    def test_portfolio_data_profiling(self, sample_portfolio_data):
        ...
        profile_results = financial_profiler.profile_portfolio_data(
            sample_portfolio_data
        )
    ...
    def test_financial_data_type_validation(self):
        ...
        validation_results = financial_profiler.validate_financial_data_types(
            test_data
        )
    ...
    @pytest.mark.skipif(
        not DATA_CLEANING_AVAILABLE, reason="Data cleaning not available"
    )
    def test_data_cleaning_basic_functionality(self, messy_financial_data):
        """Test basic data cleaning functionality."""
        cleaned_data = financial_cleaner.clean_portfolio_data(
            messy_financial_data
        )
        ...
        for item in cleaned_data:
            ...
            assert any(
                "_" in key for key in item.keys()
            )  # Should have snake_case keys

    @pytest.mark.skipif(
        not (DATA_CLEANING_AVAILABLE and PANDAS_AVAILABLE),
        reason="Data cleaning or pandas not available",
    )
    ...
    @pytest.mark.skipif(
        not (DATA_CLEANING_AVAILABLE and POLARS_AVAILABLE),
        reason="Data cleaning or polars not available",
    )
    ...
    def test_financial_value_cleaning(self):
        ...
        assert cleaned_values[1] == Decimal(
            "2500.00"
        )  # Euro sign and comma removed
        assert cleaned_values[2] == Decimal(
            "-1000.00"
        )  # Parentheses converted to negative
        assert cleaned_values[3] == Decimal("500")  # Pound sign removed
        assert cleaned_values[4] == Decimal(
            "1234.567"
    ...
    def test_cleaning_report_generation(self, messy_financial_data):
        ...
        # Generate report
        report = financial_cleaner.generate_cleaning_report(
...
class TestEndToEndDataAnalysisWorkflow:
    ...
    @pytest.mark.skipif(
        not all(
            [
                POLARS_INTEGRATION_AVAILABLE,
                DATA_CLEANING_AVAILABLE,
                DATA_PROFILING_AVAILABLE,
            ]
        ),
        ...
        if POLARS_INTEGRATION_AVAILABLE:
            ...
            for item in cleaned_data:
                portfolio_item = {
                    ...
                    "cost_basis": float(item.get("purchase_price", 0))
                    * float(item.get("shares", 0)),
                    "current_value": float(item.get("current_price", 0))
                    * float(item.get("shares", 0)),
                }
    ...
    def test_performance_comparison(self):
        ...
        if not (POLARS_AVAILABLE and PANDAS_AVAILABLE):
            pytest.skip(
                "Both Polars and Pandas needed for performance comparison"
            )
        ...
        logger.info(
            f"Performance comparison - Polars: {polars_time:.4f}s, Pandas: {pandas_time:.4f}s"
        )
