...
class TestFinancialCalculations:
    ...
    def test_sharpe_ratio_calculation(self):
        ...
        assert calculate_sharpe_ratio([0.1, 0.1, 0.1, 0.1]) is None  # Zero std dev
...
class TestPortfolioMath:
    ...
    def test_portfolio_weighted_return(self):
        ...
        # Test edge cases
        assert calculate_weighted_return([0.5, 0.5], [0.1]) is None  # Different lengths
        assert calculate_weighted_return([0.6, 0.5], [0.1, 0.2]) is None  # Weights don't sum to 1

    def test_portfolio_rebalancing(self):
        """Test portfolio rebalancing calculations."""
        
        def calculate_rebalancing_trades(current_values, target_weights, total_value):
            ...
            trades = {}
            for i, (current, target_weight) in enumerate(zip(current_values, target_weights)):
        ...
        trades = calculate_rebalancing_trades(current_values, target_weights, total_value)
...
class TestRiskMetrics:
    ...
    def test_maximum_drawdown(self):
        ...
        portfolio_values = [100000, 110000, 115000, 105000, 95000, 102000, 120000]
    ...
    def test_downside_deviation(self):
        ...
        def calculate_downside_deviation(returns, target_return=0.0):
            ...
            variance = sum((r - target_return) ** 2 for r in downside_returns) / len(downside_returns)
            return variance ** 0.5
        ...
        expected_variance = sum(r ** 2 for r in negative_returns) / len(negative_returns)
    ...
    def test_percentage_range_validation(self):
        ...
        def validate_percentage(value, min_pct=0, max_pct=100):
            ...
            if value < min_pct or value > max_pct:
                raise ValueError(f"Percentage must be between {min_pct}% and {max_pct}%")
            return True
...
class TestPerformanceMetrics:
    ...
    def test_information_ratio_calculation(self):
        ...
        def calculate_information_ratio(portfolio_returns, benchmark_returns):
            ...
            variance = sum((e - avg_excess) ** 2 for e in excess_returns) / (len(excess_returns) - 1)
