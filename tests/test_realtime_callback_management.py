...
class TestCallbackManagement:
    ...
    async def test_portfolio_subscription_with_callback(self, connection_manager, mock_callback):
        ...
        # Verify subscription
        assert portfolio_id in connection_manager.connections[connection_id]["portfolios"]
        assert connection_id in connection_manager.portfolio_subscriptions[portfolio_id]
        
        # Verify callback is registered
        callback_key = f"{connection_id}:{portfolio_id}"
        assert callback_key in connection_manager.portfolio_cleanup_callbacks
        assert mock_callback in connection_manager.portfolio_cleanup_callbacks[callback_key]

    async def test_asset_subscription_with_callback(self, connection_manager, mock_callback):
        ...
        # Verify subscription
        assert asset_symbol in connection_manager.connections[connection_id]["assets"]
        assert connection_id in connection_manager.asset_subscriptions[asset_symbol]
        
        # Verify callback is registered
        callback_key = f"{connection_id}:{asset_symbol}"
        assert callback_key in connection_manager.asset_cleanup_callbacks
        assert mock_callback in connection_manager.asset_cleanup_callbacks[callback_key]

    async def test_portfolio_unsubscribe_executes_callbacks(self, connection_manager, mock_callback, mock_callback2):
        ...
        # Unsubscribe
        await connection_manager.unsubscribe_from_portfolio(connection_id, portfolio_id)
        ...
        callback_key = f"{connection_id}:{portfolio_id}"
        assert callback_key not in connection_manager.portfolio_cleanup_callbacks

    async def test_asset_unsubscribe_executes_callbacks(self, connection_manager, mock_callback, mock_callback2):
        ...
        # Unsubscribe
        await connection_manager.unsubscribe_from_asset(connection_id, asset_symbol)
    ...
    async def test_disconnect_executes_all_callbacks(self, connection_manager, mock_callback, mock_callback2):
        ...
        assert portfolio_key not in connection_manager.portfolio_cleanup_callbacks
        assert asset_key not in connection_manager.asset_cleanup_callbacks

    async def test_subscription_without_callback_works(self, connection_manager):
        ...
        await connection_manager.subscribe_to_asset(connection_id, asset_symbol)
    ...
    async def test_multiple_connections_callback_isolation(self, connection_manager, mock_callback, mock_callback2):
        ...
        await connection_manager.unsubscribe_from_portfolio(connection_id_1, portfolio_id)
        ...
        assert callback_key_1 not in connection_manager.portfolio_cleanup_callbacks
        assert callback_key_2 in connection_manager.portfolio_cleanup_callbacks

    async def test_nonexistent_connection_unsubscribe_safe(self, connection_manager, mock_callback):
