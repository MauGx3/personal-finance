...
def test_enhanced_features():
    ...
    try:
        ...
        if all(key in metrics for key in required_keys):
            print(
                f"✅ Advanced portfolio analysis working ({len(metrics)} metrics calculated)"
            )
            success_count += 1
        else:
            print(
                f"❌ Advanced portfolio analysis missing keys: {set(required_keys) - set(metrics.keys())}"
            )
    ...
    try:
        ...
        if isinstance(search_results, list) and len(search_results) > 0:
            print(
                f"✅ Security search working ({len(search_results)} results found)"
            )
            success_count += 1
        else:
    ...
    try:
        ...
        if all(key in calendar_info for key in required_keys):
            ...
            print(
                f"❌ Market calendar info missing keys: {set(required_keys) - set(calendar_info.keys())}"
            )
    ...
    try:
        ...
        if all(key in report_data for key in required_keys):
            ...
            print(
                f"❌ Complete workflow missing keys: {set(required_keys) - set(report_data.keys())}"
            )
